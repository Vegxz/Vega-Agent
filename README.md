...
> 💡 **راهنمای مسیر تصاویر:** اگر عکس‌ها در گیت‌هاب نمایش داده نمی‌شوند، مطمئن شوید نام پوشه در ریشه‌ی مخزن دقیقاً `screenshots` (با حروف کوچک) است — گیت‌هاب و سیستم‌عامل‌های لینوکسی به بزرگی/کوچکی حروف حساس‌اند و `Screenshots` با `screenshots` یکسان شناخته نمی‌شود.
