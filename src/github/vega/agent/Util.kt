...
object Util {
    ...
    /**
     * Strips the query string and fragment from a URL for use in error
     * messages, logs and the UI. Query parameters routinely carry secrets
     * (API keys, signed download tokens — Gemini even authenticates with
     * `?key=`); the host and path are all debugging ever needs.
     */
    ...
    private val URL_WITH_SECRETS =
        Regex("(https?://[^\\s?#]+)[?#][^\\s]*")

    /**
     * Same redaction for free text: exception messages sometimes embed the
     * request URL, which would otherwise carry `?key=SECRET` onto the error
     * card verbatim.
     */
    fun redactUrlsInText(text: String?): String {
        ...
        return URL_WITH_SECRETS.replace(text) { it.groupValues[1] }
    }
}
