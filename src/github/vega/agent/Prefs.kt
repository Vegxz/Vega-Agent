...
class Prefs(context: Context) {
    ...
    /**
     * The app's own haptic feedback — the tick on button presses, switch
     * flips and sends. On by default; the Appearance group in Settings flips
     * it. Every haptic call in the app funnels through [Ui.tick], so this one
     * flag quiets all of them.
     */
    fun hapticEnabled(): Boolean = sp.getBoolean("haptic_enabled", true)

    fun setHapticEnabled(value: Boolean) {
        sp.edit().putBoolean("haptic_enabled", value).apply()
    }
    ...
    // NOTE: there is deliberately no provider/protocol setting any more. The
    // wire protocol is always detected automatically (see
    // LlmClient.protocolCandidates): the endpoint is probed and, if the first
    // guess fails, the other protocols are tried in turn. A stored "provider"
    // value from an older build is simply ignored.
    ...
    fun setApiKeys(keys: List<String>?): Boolean {
        val array = JSONArray()
        if (keys != null) {
            // The cap lives here, not just in addApiKey: a bulk paste through
            // the settings screen used to bypass the 50-key ceiling entirely and
            // grow the encrypted list without bound.
            val seen = HashSet<String>()
            var kept = 0
            for (key in keys) {
                if (kept >= MAX_ROUTER_KEYS) {
                    break
                }
                val clean = key.trimJava()
                if (clean.isEmpty() || !seen.add(clean)) {
                    continue
                }
                array.put(SecureStore.encrypt(clean))
                kept++
            }
        }
    ...
    /**
     * Explicit setting wins; otherwise the selected model's known ceiling,
     * otherwise the historical 10k. A fresh install on the default model
     * therefore starts at that model's maximum output, not at 10k.
     */
    fun maxTokens(): Int = sp.getInt("max_tokens", defaultMaxTokens())

    private fun defaultMaxTokens(): Int =
        ModelCaps.forModel(model())?.maxOutputTokens ?: 10000
    ...
    /**
     * Context window in tokens: the ceiling for system prompt + history +
     * answer in a single request. Zero means "not set explicitly" — the
     * engine then uses the selected model's known ceiling, or the default.
     * Auto-filled to the model's maximum whenever the model changes.
     */
    fun contextTokens(): Int = sp.getInt("context_tokens", 0)

    fun setContextTokens(value: Int) {
        sp.edit().putInt("context_tokens", value.coerceAtLeast(0)).apply()
    }
    ...
    /**
     * The folder the user picked as the home for agent skills, as an absolute
     * path. Empty when the user has not picked one yet — [Skills] then falls
     * back to `<workspace>/Vega Skills`. Skills themselves live in a
     * `Vega Skills` subfolder of this directory, never scattered loose in it.
     */
    fun skillsFolder(): String = sp.getString("skills_folder", "") ?: ""

    fun setSkillsFolder(value: String?) {
        sp.edit().putString("skills_folder", value?.trimJava() ?: "").apply()
    }

    /**
     * Whether the installed-skills list in Settings starts expanded. Collapsed
     * by default: the list is the noisiest thing in the Tools card and most
     * visits never touch it.
     */
    fun skillsExpanded(): Boolean = sp.getBoolean("skills_expanded", false)

    fun setSkillsExpanded(value: Boolean) {
        sp.edit().putBoolean("skills_expanded", value).apply()
    }
    ...
    fun isAnthropic(): Boolean =
        LlmClient.PROTOCOL_ANTHROPIC == LlmClient.resolveProtocol(null, baseUrl(), model())

    fun isGemini(): Boolean =
        LlmClient.PROTOCOL_GEMINI == LlmClient.resolveProtocol(null, baseUrl(), model())

    companion object {
        private const val FILE = "vega_prefs"
        ...
        private const val DEFAULT_MODEL = "gpt-6-astra"
