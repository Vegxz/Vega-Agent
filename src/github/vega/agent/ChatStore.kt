...
class ChatStore(context: Context) {
    ...
    fun load(id: String): Chat? {
        ...
        return try {
            ...
            // A corrupt chat file used to fail silently on EVERY open, so the
            // conversation was simply gone with no trace and no recovery. Move
            // it aside (nothing is deleted) and let the caller start fresh.
    ...
    /**
     * Moves an unreadable chat file out of the way, keeping its bytes.
     *
     * The backup name carries no `.json` suffix on purpose: the listing loops
     * only look at `*.json`, so a quarantined file is never re-parsed (and
     * never re-quarantined) on the next refresh.
     */
    private fun quarantineCorrupt(file: File) {
        try {
            if (!file.name.endsWith(".json") || !file.exists()) {
                return
            }
            val backup = File(dir, file.nameWithoutExtension +
                ".corrupt-" + System.currentTimeMillis())
            if (!file.renameTo(backup)) {
                Log.w("Vega", "could not quarantine corrupt chat " + file.name)
            }
        } catch (e: Exception) {
            Log.w("Vega", "could not quarantine corrupt chat " + file.name, e)
        }
    }
    ...
    private fun readSummary(file: File): Summary? {
        ...
        // A file that fails even that is corrupt: quarantine it (see load()) so
        // the drawer does not pay for a doomed parse on every refresh.
        return try {
    ...
    /** Most recently updated first. Corrupt files are quarantined, not fatal. */
    ...
    companion object {
        ...
        private val writer: ExecutorService = Executors.newSingleThreadExecutor { runnable ->
            val thread = Thread(runnable, "vega-chat-store")
