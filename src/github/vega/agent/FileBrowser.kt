...
class FileBrowser(private val c: Context, private val onPick: OnPick?) {
    ...
    /** Row inflation for [refresh]: always runs on the main thread. */
    ...
    private fun refresh() {
        ...
        // Directory I/O off the main thread: a Downloads folder with thousands
        // of entries made listFiles() + sort drop frames (ANR on slow eMMC).
        // The crumb and the pinned rows above are already on screen; only the
        // listing itself goes to the background.
        val dir = cur
        ...
        diskIo.execute {
            ...
            main.post {
                // Stale: the user navigated again while this listing ran.
                if (generation != listGeneration || cur != dir) {
                    return@post
                }
    ...
    /** Row inflation for [refresh]: always runs on the main thread. */
    private fun renderChildren(children: MutableList<File>?) {
    ...
    companion object {
        /**
         * Directory listing runs here, never on the main thread: listFiles()
         * on a crowded folder drops frames (ANR on slow eMMC). Shared across
         * instances (a browser is created per sheet open) and single-threaded
         * so two rapid navigations cannot interleave; the generation guard in
         * [refresh] drops the stale one anyway.
         */
