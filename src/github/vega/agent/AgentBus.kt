...
object AgentBus {
    ...
    /**
     * B3: when the oldest still-outstanding approval was queued, or 0 while no
     * approval is waiting. The run's no-output watchdog exempts this span — a
     * person reading a diff for eleven minutes is parked on the user, not a
     * stalled run. The 90-minute wall clock still applies; this only covers the
     * no-output ceiling. Guarded by [RUN_LOCK] for writes; `@Volatile` for the
     * watchdog's reads.
     */
    @Volatile
    private var approvalParkedAt = 0L

    /**
     * B3: when the oldest still-outstanding approval was queued (0 when none is
     * waiting). Lets the run's no-output deadline be suspended while the agent
     * is parked on the user's answer.
     */
    fun approvalParkedSince(): Long = approvalParkedAt

    /**
     * B3: when the last parked approval SETTLED (decided, rejected, or timed
     * out), or 0 if none has. Answering the user is progress: the run's
     * no-output deadline resumes from the answer instead of firing on the
     * stale pre-ask timestamp. `@Volatile` for the watchdog's reads; written
     * under [RUN_LOCK] in [retire] and [drainLocked].
     */
    @Volatile
    private var lastApprovalSettledAt = 0L

    /** B3: see [lastApprovalSettledAt]. */
    fun lastApprovalSettledAt(): Long = lastApprovalSettledAt
    ...
    interface UiListener {
        ...
        /**
         * An approval was decided. Fires for EVERY decision — including ones
         * made by a timeout, a cancellation, or the run ending, where no sheet
         * button was ever tapped.
         *
         * That last case is the whole reason this exists: when the bus retires
         * an approval the UI never hears about it otherwise, and the sheet
         * stays on screen forever asking a question nobody can answer any
         * more. Optional with a body like [onApprovalQueueChanged].
         */
        fun onApprovalDecided(approval: PendingApproval) {
        }
    }
    ...
    class PendingApproval internal constructor(
        ...
        /** B3: when this ask was queued — the oldest one anchors [approvalParkedAt]. */
        internal val queuedAt: Long = System.currentTimeMillis()
        ...
        internal fun decide(approved: Boolean, why: String) {
            ...
            if (changed) {
                retire(this)
                // No locks held here: retire() took and released RUN_LOCK
                // already. The UI must learn about EVERY decision — a timeout
                // or cancellation retires an approval whose sheet is still on
                // screen, and without this the sheet never goes away.
                listener?.onApprovalDecided(this)
            }
        }
    }
    ...
    fun awaitApprovalDetailed(
        ...
        val approval = synchronized(RUN_LOCK) {
            ...
            // B3: the oldest outstanding ask anchors the parked span.
            if (approvalParkedAt == 0L || queued.queuedAt < approvalParkedAt) {
                approvalParkedAt = queued.queuedAt
            }
            queued
        }
    ...
    private fun drainLocked(): List<PendingApproval> {
        // B3: nothing is parked on the user once the queue is gone. The settle
        // timestamp still advances: a cancel that lands between the drain and
        // the watchdog closing must not trip the no-output ceiling on the way
        // out.
        approvalParkedAt = 0L
        lastApprovalSettledAt = System.currentTimeMillis()
    ...
    internal fun retire(approval: PendingApproval) {
        ...
        val removed = synchronized(RUN_LOCK) {
            val gone = approvals.remove(approval)
            if (gone) {
                // B3: re-anchor the parked span on the oldest ask that is still
                // waiting; 0 when the queue is empty again.
                var oldest = 0L
                for (pending in approvals) {
                    if (!pending.decided &&
                        (oldest == 0L || pending.queuedAt < oldest)
                    ) {
                        oldest = pending.queuedAt
                    }
                }
                approvalParkedAt = oldest
                // B3: the ask settled — the answer counts as progress, so the
                // no-output deadline resumes from here rather than from the
                // stale timestamp of whatever happened before the ask.
                // `decide` retires exactly once (first decision wins), so the
                // first settlement is what anchors the resume.
                lastApprovalSettledAt = System.currentTimeMillis()
            }
            gone
        }
