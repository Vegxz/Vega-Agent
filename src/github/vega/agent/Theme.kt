 ...
 *
 * It replaces the older tinted look with a near-monochrome one, built on
 * four ideas:
 *
 *  1. **Near-zero chroma.** Every token is black, white, or a grey between
 *     them — except the settings menu's icon tiles ([tileBlue] et al), which
 *     carry iOS's own system hues in the register of iOS Settings: a solid
 *     coloured tile with a white glyph. The dark spine is near-black graphite,
 *     the light spine an iOS-style grouped page — a cool grey the white cards
 *     sit on as objects. Meaning that used to be carried by hue is re-encoded
 *     as lightness, typeface weight, glyph choice and a 2dp start rail.
...
object Theme {
    ...
    var SURFACE_2: Int = 0
    /**
     * The most-raised layer: chips, badge grounds and wells nested inside
     * [SURFACE_2]. One step above it in both palettes, so a control sitting on
     * a card still reads as an object rather than as paint.
     */
    var SURFACE_3: Int = 0
    /**
     * Table header tint and zebra stripe — the hooks the table renderer layers
     * over its card. [TABLE_HEAD_BG] sits one step above the card's fill so the
     * header reads as a band; [TABLE_STRIPE] sits one step off it for the
     * alternating body row. Pure greys, like everything else here.
     */
    var TABLE_HEAD_BG: Int = 0
    var TABLE_STRIPE: Int = 0
    /** The softest hairline: for large quiet containers whose edge should whisper. */
    var BORDER_SOFT: Int = 0
    var TEXT: Int = 0
    ...
    // ---- iOS settings-tile hues -------------------------------------------
    //
    // The one place this system carries hue outside diffs: the settings menu's
    // icon tiles, in the register of iOS Settings — a solid rounded square in
    // a system colour with a white glyph. Each follows iOS's own dark-mode
    // lightening, so the tile reads the same against both grounds.

    /**
     * The glyph on a coloured settings tile — always white, in both themes.
     * The tile carries the colour and the glyph carries the shape, so it never
     * inverts: a black glyph would sink into the darker tiles, and a themed
     * one would argue with the tile's own hue.
     */
    val TILE_GLYPH: Int = 0xFFFFFFFF.toInt()

    /** iOS system blue — connectivity, tools. */
    fun tileBlue(): Int = if (DARK) 0xFF0A84FF.toInt() else 0xFF007AFF.toInt()

    /** iOS system green — confirmations, secure states. */
    fun tileGreen(): Int = if (DARK) 0xFF30D158.toInt() else 0xFF34C759.toInt()

    /** iOS system orange — appearance, personalisation. */
    fun tileOrange(): Int = if (DARK) 0xFFFF9F0A.toInt() else 0xFFFF9500.toInt()

    /** iOS system red — destructive actions. */
    fun tileRed(): Int = if (DARK) 0xFFFF453A.toInt() else 0xFFFF3B30.toInt()

    /** iOS system purple — intelligence, creativity. */
    fun tilePurple(): Int = if (DARK) 0xFFBF5AF2.toInt() else 0xFFAF52DE.toInt()

    /** iOS system teal — information, sync. */
    fun tileTeal(): Int = if (DARK) 0xFF64D2FF.toInt() else 0xFF5AC8FA.toInt()

    /** iOS system grey — neutral destinations. */
    fun tileGray(): Int = if (DARK) 0xFF98989D.toInt() else 0xFF8E8E93.toInt()

    /** iOS system pink — highlights. */
    fun tilePink(): Int = if (DARK) 0xFFFF375F.toInt() else 0xFFFF2D55.toInt()

    /** iOS system indigo — accounts, identity. */
    fun tileIndigo(): Int = if (DARK) 0xFF5E5CE6.toInt() else 0xFF5856D6.toInt()

    /** iOS system yellow — warnings, energy. */
    fun tileYellow(): Int = if (DARK) 0xFFFFD60A.toInt() else 0xFFFFCC00.toInt()
    ...
    fun apply(dark: Boolean) {
        ...
        if (dark) {
            ...
            SURFACE_3 = 0xFF2F2F2F.toInt()
            TABLE_HEAD_BG = 0xFF2E2E2E.toInt()
            TABLE_STRIPE = 0xFF292929.toInt()
            BORDER_SOFT = 0xFF1F1F1F.toInt()
        ...
        // Paper — the light spine and the reference theme. An iOS-style grouped
        // page (systemGray6): a cool grey ground the white cards sit on as
        // objects, the way iOS Settings reads. The page is a full step darker
        // than the cards, never the reverse.
        BG = 0xFFF2F2F7.toInt()
        BG_ELEV = 0xFFF2F2F7.toInt()
        SURFACE = 0xFFFFFFFF.toInt()
        SURFACE_2 = 0xFFF0F0F0.toInt()
        SURFACE_3 = 0xFFE6E6E6.toInt()
        TABLE_HEAD_BG = 0xFFE7E7E7.toInt()
        TABLE_STRIPE = 0xFFF4F4F4.toInt()
        BORDER_SOFT = 0xFFEFEFEF.toInt()
    ...
    /**
     * The background for rendered pipe tables.
     *
     * The recessed [sunkenCard] surface with a firmer [BORDER_HI] hairline: a
     * table is a structural grid, and its edge needs one notch more definition
     * than a code card's. Same 1dp inset geometry as every other card here, so
     * it is a drop-in. The header tint ([TABLE_HEAD_BG]) and the zebra stripe
     * ([TABLE_STRIPE]) are layered on top by the renderer, not baked in here.
     */
    fun tableCard(radius: Float, context: Context): GradientDrawable =
        roundStroke(SURFACE_2, BORDER_HI, radius, 1, context)
    ...
    fun elevate(view: View, elevationDp: Float, radius: Float, clipChildren: Boolean) {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            //
            // Kept deliberately soft and light, iOS-style: a shadow should
            // lift a surface, not draw a dark rim around it.
            if (DARK) {
                view.outlineAmbientShadowColor = 0x2E000000
                view.outlineSpotShadowColor = 0x52000000
            } else {
                view.outlineAmbientShadowColor = 0x0F000000
                view.outlineSpotShadowColor = 0x1A000000
            }
        }
    }
