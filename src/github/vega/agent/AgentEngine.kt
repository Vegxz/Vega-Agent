 ...
 * feed the result back, repeat — up to the effort-scaled step budget.
 *
...
class AgentEngine(
    ...
    interface Callback {
        ...
        /**
         * Asks the user whether [tool] may run. The full [AgentBus.Decision]
         * comes back — not just a Boolean — so the run can tell a rejection
         * apart from a timeout or a cancellation and say so to the model.
         */
        fun requestApproval(tool: String, args: JSONObject?): AgentBus.Decision
    }
    ...
    /**
     * Run-scoped state. One engine instance serves one run, so these are
     * initialised at the top of [run] rather than threaded through locals.
     */

    /** Thinking-level boost from magic keywords; computed from the last user message at run start. */
    private var thinkingBoost: String? = null

    /** Cached system prompt and the key it was built from (B5). */
    private var cachedSystemPrompt: String? = null
    private var cachedSystemPromptKey: String? = null

    /** The session plan file for this run (PLAN-mode only, depth 0). */
    private var activePlanFile: File? = null

    /** Verification gate: phases closed since the last verification, and whether the nudge is armed. */
    private var unverifiedCloses = 0

    @Volatile
    private var verificationNudgeDue = false

    /** Cached compaction summary and the dropped-prefix length it describes. */
    private var compactSummary: String? = null
    private var compactSummaryStart = -1

    /** How many plan-mode turns have elapsed (drives the 5-turn reminder cadence). */
    private var planTurn = 0
    ...
    fun run(chat: Chat, token: CancellationToken, callback: Callback) {
        ...
        // Wall clock for the run ceilings below (wall-clock and no-output).
        ...
        // Run-scoped state (instance fields: one engine instance serves one run).
        // Magic-keyword thinking boost from the user's message ("ultrathink",
        // "think harder", ...); only ever raises the configured level.
        thinkingBoost = thinkingKeywordLevel(lastUserText(chat))
        unverifiedCloses = 0
        verificationNudgeDue = false
        compactSummary = null
        compactSummaryStart = -1
        planTurn = 0
        activePlanFile = if (depth == 0) sessionPlanFile(chat.id) else null
        // The plan file's folder must exist BEFORE the model ever calls a tool:
        // PLAN mode refuses make_dir (read-only), so without this the model's
        // first make_dir .vega/plans call fails, list_dir reports "not found",
        // and the run burns turns discovering a folder the app could have
        // created itself. Eager creation removes the whole failure class.
        activePlanFile?.parentFile?.let { plansDir ->
            try {
                if (!plansDir.isDirectory) {
                    plansDir.mkdirs()
                }
            } catch (ignored: Exception) {
            }
        }
        // Approved-plan handoff (ExitPlanMode semantics): the plan sheet flips
        // the stored mode to ACCEPT when the user taps "Run plan". The next run
        // picks the approved plan up from the session plan file and starts from
        // it — exactly once, tracked by the .consumed marker.
        if (depth == 0 && Prefs.MODE_ACCEPT == planMode) {
            activePlanFile?.let { plan ->
                readApprovedPlan(plan)?.let { approved ->
                    nudges.add(approvedPlanHandoff(plan, approved))
                    markPlanConsumed(plan)
                }
            }
        }
        ...
        // B8: no run-start reset of the wanted-changes signal. The old
        // `markWantedChanges(false)` here erased a refusal the UI had not
        // consumed yet — a follow-up run in the same chat cleared the bit
        // before the plan sheet ever read it. The signal now lives until it is
        // explicitly consumed (the property read below), so a refusal is never
        // silently dropped by the next run starting.
        ...
        /**
         * Set by the run watchdog when a time ceiling actually fires, BEFORE
         * it cancels the token — so the stop path below can end the run with
         * that ceiling's marker (which the Continue card recognises) instead
         * of a bare stop. Null when the user stopped the run themselves.
         */
        val watchdogFired = java.util.concurrent.atomic.AtomicReference<String?>(null)

        /**
         * The run's true deadlines. Armed after preflight for the top-level
         * run only (a sub-agent shares its parent's token, so the parent's
         * watchdog already bounds it).
         *
         * Declared up here — before it is armed — because settleAll() closes
         * it, and every terminal path of run() goes through settleAll().
         */
        var watchdog: RunWatchdog? = null
        ...
        fun settleAll(interrupted: Boolean = false) {
            ...
            watchdog = null
            // B15: collapse on EVERY terminal path, not just the genuine-answer
            // one. An error, a stop, or an exhausted retry budget ends the run
            // just as finally as an answer does — the strip collapses to its
            // "thought for N seconds" summary either way, and only the answer
            // path's panel narration differs.
        ...
        // ---- run ceilings --------------------------------------------------
        //
        // The step and fault budgets bound WORK; these bound TIME. A provider
        // that streams nothing for ten minutes is not thinking, and a run older
        // than ninety minutes is not coming back — both used to hang the run
        // until the user pressed stop. `runStartedAt` above is the wall clock.
        val lastProgressMs = java.util.concurrent.atomic.AtomicLong(runStartedAt)

        /**
         * B3: the baseline the no-output deadline is measured from. Answering
         * a parked approval counts as progress, so the deadline resumes from
         * the answer instead of firing on the stale pre-ask timestamp.
         */
        fun progressBaseline(): Long =
            Math.max(lastProgressMs.get(), AgentBus.lastApprovalSettledAt())

        /**
         * B3: the no-output deadline is SUSPENDED while an approval is parked
         * on the user — a person reading a diff for eleven minutes is parked
         * on the user, not a stalled run. Suspension is absolute, not a
         * baseline shift: shifting the baseline to the queue time would still
         * fire on a long enough wait. The wall clock is unaffected: approval
         * waiting is idle, and idle still counts against the run's budget.
         */
        fun noOutputSuspended(): Boolean = AgentBus.approvalParkedSince() > 0L

        /**
         * Ends the run with a "did not finish" marker the continue card
         * recognises. Mirrors the stall path: the message is a real assistant
         * turn so the transcript stays honest about where the run stopped.
         */
        fun endRunUnfinished(text: String) {
            val marker = Message("assistant", text)
            synchronized(chat.messages) {
                chat.messages.add(marker)
            }
            settleAll(interrupted = true)
            callback.onNewAssistantMessage(marker)
            callback.onStepFinalized(marker)
            callback.onComplete()
        }

        // Sub-agent heartbeats feed the lead's silence watchdog: a wave of
        // three agents can legitimately take twenty minutes, and without this
        // the lead would conclude nothing was happening ten minutes in.
        val publishWithProgress: () -> Unit = {
        ...
        // ---- context budgeting -------------------------------------------
        //
        // The 120k ceiling used to cover HISTORY ONLY: the system prompt and
        // the output reserve rode on top of it, so a request could exceed the
        // provider's window while the history looked safely budgeted. The
        // budget is now computed from the whole request — system prompt +
        // history + output reserve — and the overflow fast path below shrinks
        // it further when the provider still says the request is too large.
        val systemChars = systemPrompt().length
        val outputReserveChars = (if (prefs.maxTokens() > 0) prefs.maxTokens() else 4096) * 4
        // The budget follows the selected model: an explicit context-window
        // setting wins, then the model's known ceiling ([ModelCaps]), then
        // the historical default. Switching to a 1M-token model actually
        // gives the run 1M tokens of room instead of the old fixed 120k chars.
        var historyChars = (ModelCaps.inputChars(prefs) - systemChars - outputReserveChars)
            .coerceAtLeast(MIN_HISTORY_CHARS)
        var overflowShrinks = 0
        ...
        // Arm the true deadlines. The step-loop checks below only run BETWEEN
        // steps; a single blocked streamChat, tool call, or sub-agent
        // delegation would otherwise sit past them indefinitely. The watchdog
        // cancels the token when a ceiling actually expires, which unwinds the
        // streaming IO thread, interrupts a wedged tool worker, and stops a
        // sub-agent — every layer already observes the token.
        if (depth == 0) {
            watchdog = RunWatchdog(
                token = token,
                fired = watchdogFired,
                wallClockAtMs = runStartedAt + RUN_WALL_CLOCK_MS,
                noOutputMs = NO_OUTPUT_MS,
                lastProgressMs = lastProgressMs,
                wallClockReason = Fa.RUN_WALL_CLOCK,
                noOutputReason = Fa.RUN_NO_OUTPUT
            )
        }

        /**
         * Leaves the run at a cancellation point. When the watchdog fired, the
         * run ends with that ceiling's marker — a real assistant turn, so the
         * Continue card recognises it — instead of a bare stop.
         */
        fun stopRun() {
            val woken = watchdogFired.get()
            if (woken != null) {
                endRunUnfinished(woken)
            } else {
                settleAll(interrupted = true)
                callback.onComplete()
            }
        ...
        // ---- tool dispatch -------------------------------------------------
        //
        // The single-call path below and the multi-call batch share one
        // dispatcher ([executeOneCall]) and one observer factory
        // ([makeWatcher]), so the approval behaviour, the plan-mode guards,
        // the delegation rules and the trail bookkeeping cannot drift apart
        // between the two paths.

        /**
         * PLAN mode's second allowed write: creating the session plan file's
         * own folder. The folder is created eagerly at run start, but the
         * model may still call make_dir for it (or a parent of it) before
         * writing — refusing that call while allowing the write it enables
         * is a contradiction that burned real turns.
         *
         * Declared BEFORE [planBlockedFor]: local functions cannot be
         * forward-referenced.
         */
        fun isPlanDirMake(call: ToolCall): Boolean {
            if (Tools.ToolNames.MKDIR != call.name) {
                return false
            }
            val planFile = activePlanFile ?: return false
            val raw = call.args.optStr("path", "")
            if (raw.isBlankJava()) {
                return false
            }
            return try {
                val target = resolveUnderRoot(raw).canonicalPath
                val planDir = planFile.parentFile?.canonicalPath ?: return false
                target == planDir || planDir.startsWith(target + File.separator)
            } catch (e: Exception) {
                false
            }
        }
        ...
        /**
         * B14 + session plan file: PLAN mode refuses mutating calls — EXCEPT
         * `remember` (memory is not a file change) and writes to this run's
         * session plan file, which is the one path PLAN mode may write.
         */
        fun planBlockedFor(call: ToolCall): Boolean {
            if (Prefs.MODE_PLAN != planMode || !Tools.isMutating(call.name)) {
                return false
            }
            if (Tools.ToolNames.REMEMBER == call.name) {
                return false
            }
            return !isPlanFileWrite(call) && !isPlanDirMake(call)
        }


        /**
         * Verification-gate bookkeeping: re-reading the work IS verification,
         * so any read-only file inspection resets the unverified-close
         * counter. (Web lookups are not verification of the changed files.)
         */
        fun noteToolExecuted(name: String?) {
            if (depth != 0) {
                return
            }
            when (name) {
                Tools.ToolNames.READ_FILE,
                Tools.ToolNames.LIST_DIR,
                Tools.ToolNames.FILE_INFO,
                Tools.ToolNames.SEARCH,
                Tools.ToolNames.GLOB,
                Tools.ToolNames.READ_PDF,
                Tools.ToolNames.READ_ARCHIVE_ENTRY,
                Tools.ToolNames.LIST_ARCHIVE -> noteVerified()
            }
        }

        /**
         * Anything the tool learns that is worth SHOWING rather than telling
         * the model goes straight onto this step's row. Null when there is no
         * trail to watch.
         */
        fun makeWatcher(step: TrailStep): Tools.Observer? {
            val trailRef = trail ?: return null
            return object : Tools.Observer {
                /**
                 ...
                 */
                private fun live(): Boolean = step.status == TrailStep.RUNNING

                override fun onSearchResults(results: List<Web.SearchResult>) {
                    if (!live()) {
                        return
                    }
                    step.addResults(results)
                    ...
                    trailRef.addPages(hosts)
                    if (step.resultCount <= 0) {
                        step.resultCount = results.size
                    }
                    publishTrail()
                }
                ...
                override fun onProgress(detail: String) {
                    ...
                    step.detail = clip(detail, DETAIL_CHARS)
                    publishTrail()
                }
        ...
        /**
         * Runs one tool call through every guard in the same order the inline
         * dispatch always used: plan refusal, task-in-plan refusal,
         * delegation, approved batch deletes, the approval branch, task
         * delegation, and finally the tool itself.
         */
        fun executeOneCall(
            call: ToolCall,
            planBlocked: Boolean,
            planTaskBlocked: Boolean,
            mustDelegate: Boolean,
            mode: String,
            watcher: Tools.Observer?
        ): String = when {
            // Refused, not escalated. See the note above the guard.
            planBlocked -> PLAN_REFUSAL

            // B9: the task tool is not available in PLAN mode — a delegation
            // would inherit the read-only mode and refuse every change, so it
            // buys nothing and the model should investigate itself.
            planTaskBlocked -> TASK_PLAN_REFUSAL
            ...
            // In ACCEPT/PLAN mode every action asks first — including
            // `task` delegations, which sit below this branch on
            // purpose. (AUTO mode skips this branch, so nothing
            // changes there.)
            asksPermission(mode) && Tools.needsApproval(call.name) &&
                !isAllowedForSession(call.name) ->
                run {
                    val decision = callback.requestApproval(call.name, call.args)
                    if (decision.approved) {
                        if (TASK_TOOL == call.name) {
                            runSubAgents(
                                call.args, chat, token, callback, workflow,
                                publishWithProgress
                            )
                        } else {
                        ...
                        approvalRefusalText(decision.reason)
                    }
                }

            // Dynamic Workflow: delegate a whole sub-task to a focused
            // sub-agent with its own clean context. Reached only when
            // the approval branch above did not fire (AUTO mode, or a
            // session allowance) — in ACCEPT/PLAN the delegation asks
            // first up there.
            TASK_TOOL == call.name -> runSubAgents(
                call.args, chat, token, callback, workflow, publishWithProgress
            )
        ...
        /**
         * The plan-mode guard inputs for one call. Shared by the batch so a
         * mixed batch (reads next to writes) refuses exactly the calls PLAN
         * mode forbids and no others.
         */
        fun callGuards(call: ToolCall): Triple<Boolean, Boolean, Boolean> {
            val blocked = planBlockedFor(call)
            val taskBlocked = Prefs.MODE_PLAN == planMode && TASK_TOOL == call.name
            // The lead's own bookkeeping is not a work phase: `remember` notes
            // and writes to the session plan file must never be delegated — a
            // sub-agent cannot write the lead's memory or its plan for it, so
            // DELEGATE_REFUSAL here would refuse work nobody else can do.
            val delegate = depth == 0 && !blocked &&
                Tools.ToolNames.REMEMBER != call.name && !isPlanFileWrite(call) &&
                prefs.dynamicWorkflow() && Tools.isMutating(call.name)
            return Triple(blocked, taskBlocked, delegate)
        }

        /**
         * Waits for one parallel tool future in short slices (B1's discipline
         * applied to the batch pool): a stop is honoured within ~2s instead of
         * whenever the tool decides to return. Returns null when the run was
         * stopped while waiting.
         */
        fun awaitToolFuture(future: java.util.concurrent.Future<String>): String? {
            while (true) {
                ...
                try {
                    ...
                    // Slice elapsed — loop around and re-check the token.
                } catch (e: InterruptedException) {
                    // Never swallowed: re-interrupt so the run's stop path
                    // sees it, and let it propagate as a fault.
                    Thread.currentThread().interrupt()
                    throw e
                } catch (e: java.util.concurrent.ExecutionException) {
                    // Unwrap: the run's fault path must see the REAL exception
                    // (and a real Error), never the wrapper.
                    val cause = e.cause
                    if (cause is Error) {
                        throw cause
                    }
                    throw cause as? Exception ?: Exception(cause)
                }
            }
        }

        /**
         * Multi-call batch (Claude Code's tool loop): one message, N
         * independent ```json blocks.
         *
         * The concurrency-safe (read-only) calls run in parallel on a bounded
         * daemon pool (max 4 workers); everything else runs serially in model
         * order. Results are collected strictly in model order and each is
         * delivered as its own [TOOL RESULT: name] tool message — the contract
         * the system prompt promises.
         *
         * A batch may mix allowed and refused calls (reads next to writes in
         * PLAN mode): the per-call guards run per call, and each refusal
         * signals the plan sheet on its own.
         */
        fun runToolCallBatch(calls: List<ToolCall>) {
            val mode = prefs.mode()
            val results = arrayOfNulls<String>(calls.size)
            val steps = arrayOfNulls<TrailStep>(calls.size)
            for (i in calls.indices) {
                val one = calls[i]
                callback.onToolRunning(one.name, summarizeArgs(one.args))
                steps[i] = trail?.let { openTrailStep(it, one) }
            }
            if (trail != null) {
                publishTrail()
            }
            ...
            try {
                // Phase 1: submit the concurrency-safe calls to the pool.
                val futures = HashMap<Int, java.util.concurrent.Future<String>>()
                var safeCount = 0
                for (one in calls) {
                    if (isConcurrencySafe(one.name)) {
                        safeCount++
                    }
                }
                if (safeCount > 0) {
                    val workers =
                        java.util.concurrent.Executors.newFixedThreadPool(
                            Math.min(4, safeCount)
                        ) { work ->
                            Thread(work, "vega-tool-batch").also { it.isDaemon = true }
                        }
                    pool = workers
                    for (i in calls.indices) {
                        val one = calls[i]
                        ...
                        val (blocked, taskBlocked, delegate) = callGuards(one)
                        if (blocked && depth == 0) {
                            markWantedChanges()
                        }
                        val watcher = steps[i]?.let { makeWatcher(it) }
                        futures[i] = workers.submit(
                            java.util.concurrent.Callable<String> {
                                executeOneCall(
                                    one, blocked, taskBlocked, delegate, mode, watcher
                                )
                            }
                        )
                    }
                }
                // Phase 2: collect strictly in model order, running the rest
                // serially as each index is reached.
                for (i in calls.indices) {
                    val one = calls[i]
                    val future = futures[i]
                    val result = if (future != null) {
                        awaitToolFuture(future)
                    } else {
                        val (blocked, taskBlocked, delegate) = callGuards(one)
                        if (blocked && depth == 0) {
                            markWantedChanges()
                    ...
                    results[i] = result
                    noteToolExecuted(one.name)
                    val step = steps[i]
                    if (step != null && trail != null) {
                        closeTrailStep(trail, step, result)
                        publishTrail()
                    }
                    lastProgressMs.set(System.currentTimeMillis())
                }
            ...
            // One tool message per completed call, in model order.
            for (i in calls.indices) {
                val result = results[i] ?: continue
                val toolMessage = Message("tool", result)
                toolMessage.toolLog.add(calls[i].name)
                // Folded into the strip like the prose that requested it.
                toolMessage.isStep = trail != null
                synchronized(chat.messages) {
                    chat.messages.add(toolMessage)
                }
                callback.onToolMessage(toolMessage, calls[i].name)
            }
            produced = true
            // A finished batch is progress, even when it produced no tokens.
            lastProgressMs.set(System.currentTimeMillis())
        }
        ...
        val stepLimit = if (depth > 0) {
            maxSubagentStepsForLevel(prefs.effectiveThinkingLevel())
        } else {
            maxStepsForLevel(prefs.effectiveThinkingLevel())
        }
        for (step in 0 until stepLimit) {
            try {
                ...
                // The step and fault budgets bound work; these bound time.
                val nowMs = System.currentTimeMillis()
                if (nowMs - runStartedAt > RUN_WALL_CLOCK_MS) {
                    endRunUnfinished(Fa.RUN_WALL_CLOCK)
                    return
                }
                if (!noOutputSuspended() && nowMs - progressBaseline() > NO_OUTPUT_MS) {
                    endRunUnfinished(Fa.RUN_NO_OUTPUT)
                    return
                }

                // Per-turn drivers, injected as <system-reminder> USER messages
                // (Claude Code's attachment trick): the cached system prompt is
                // never busted, and each reminder is small and cache-friendly.
                if (Prefs.MODE_PLAN == planMode) {
                    // Plan mode's 5-phase workflow: the full driver every 5th
                    // turn, a one-line sparse reminder between turns.
                    planTurn++
                    nudges.add(planModeReminder(planTurn))
                }
                if (verificationNudgeDue) {
                    // Verification gate: 3+ phases closed without a single
                    // re-read of the changed work — verify before declaring
                    // victory.
                    verificationNudgeDue = false
                    nudges.add(VERIFICATION_NUDGE)
                }

                val apiMessages = buildApiMessages(chat, nudges, historyChars, token)
                ...
                // B4: message.content/thinking used to be rebuilt from the two
                // buffers on EVERY token — O(n²) copying on a long answer, for
                // data the UI only reads as deltas. Sync every STREAM_SYNC_EVERY
                // tokens instead; the final content is still synced exactly at
                // round end, and any path that needs the current text calls
                // syncStreamContent() first.
                var streamSyncCountdown = STREAM_SYNC_EVERY
                fun syncStreamContent() {
                ...
                val stream = object : LlmClient.StreamCallback {
                    override fun onToken(text: String) {
                        ...
                        produced = true
                        // B4: periodic sync, not per token.
                        if (--streamSyncCountdown <= 0) {
                            streamSyncCountdown = STREAM_SYNC_EVERY
                            syncStreamContent()
                        }
                ...
                try {
                    ...
                    // B4: the post-loop sync below is skipped when streamChat
                    // throws — the buffers are still authoritative, so sync here
                    // too rather than leaving a stale periodic copy behind.
                    syncStreamContent()
                }
                ...
                if (error != null) {
                    // ---- context overflow: the deterministic fast path -------
                    //
                    // The provider rejected the REQUEST ITSELF as too large, so
                    // retrying the identical request fails identically — the
                    // widening backoff and the fault budget would just convert a
                    // clear, fixable condition into minutes of doomed retries.
                    // Halve the history budget and rebuild the request at once,
                    // without spending a fault recovery on it.
                    if (isContextOverflow(error) && overflowShrinks < MAX_OVERFLOW_SHRINKS) {
                        overflowShrinks++
                        historyChars = (historyChars / 2).coerceAtLeast(MIN_HISTORY_CHARS)
                        trail?.let {
                            it.phase = Fa.RUN_COMPACTING
                        ...
                        // B7: the shrink starts a fresh turn, so stale corrections
                        // must not ride along — including NUDGE_AFTER_FAULT from
                        // an earlier recovery. (The per-turn drivers are re-added
                        // at the top of the loop.)
                        nudges.clear()
                        // A beat for cancellation to land; the next loop rebuilds
                        // the request with the smaller budget.
                        if (!token.sleep(400L)) {
                            settleAll(interrupted = true)
                            callback.onComplete()
                            return
                    ...
                    // B6: the fast path already halved the history budget three
                    // times and the provider STILL rejects the request as too
                    // large. Shrinking further (or falling through to the six
                    // generic fault recoveries) only buys doomed retries on a
                    // request the provider has already refused — fail fast with
                    // a clear reason and a way out.
                    if (isContextOverflow(error)) {
                        ...
                        val tooLarge = Message("assistant", CONTEXT_TOO_LARGE)
                        tooLarge.isError = true
                        synchronized(chat.messages) {
                            chat.messages.add(tooLarge)
                        }
                        callback.onNewAssistantMessage(tooLarge)
                        callback.onStepFinalized(tooLarge)
                        callback.onComplete()
                        return
                    }
                    ...
                    if (faults < budget && !stopped(token, callback)) {
                        ...
                        if (!token.sleep(faultSleepMs(faults))) {
                            settleAll(interrupted = true)
                            callback.onComplete()
                            return
                        }
                ...
                val calls = parseToolCalls(message.content)
                // The single-call path below works on the LAST call — the
                // model's final decision — exactly as parseToolCall did. When
                // the message holds several independent calls, the batch path
                // runs them all instead.
                val call = calls.lastOrNull()
                ...
                if (call == null) {
                    ...
                    if (visible.isEmpty() && !stopped(token, callback)) {
                        ...
                        if (stalls < MAX_STALL_RECOVERIES) {
                            stalls++
                            nudges.clear()
                ...
                // Multi-call batch (Claude Code's tool loop): one message, N
                // independent ```json blocks. The board rules are the same as
                // the single-call path's — created on the first tool call of a
                // Dynamic Workflow run, ensured for task delegations.
                if (calls.size > 1) {
                    if (trail != null) {
                        thoughtCursor = flushThinking(
                            trail, effectiveReasoning(reasoning, body), thoughtCursor
                        )
                    }
                    ...
                    if (calls.any { one -> TASK_TOOL == one.name } &&
                        depth == 0 && prefs.dynamicWorkflow()
                    ) {
                        if (workflow == null) {
                            workflow = buildWorkflow(chat)
                            // `announced` as the fallback: `trailOwner` is
                            // cleared every time a step is dropped and the trail
                            // re-homed, and a board attached to nothing is never
                ...
                //
                // B14: `remember` is exempt — memory is not a file change. The
                // session plan file is the one path PLAN mode may write.
                val planBlocked = planBlockedFor(call)
                if (planBlocked) {
                    if (depth == 0) {
                        markWantedChanges()
                    }
                }
                // B9: the task tool is refused outright in PLAN mode — a
                // delegation would inherit the read-only mode and refuse every
                // change, so delegation buys nothing here.
                val planTaskBlocked = Prefs.MODE_PLAN == planMode && TASK_TOOL == call.name
                ...
                val watcher = trailStep?.let { makeWatcher(it) }

                val result: String = executeOneCall(
                    call, planBlocked, planTaskBlocked, mustDelegate, mode, watcher
                )
                // Verification gate: re-reading the work counts as verifying it.
                noteToolExecuted(call.name)
                ...
                // A finished tool call is progress, even when it produced no
                // tokens: a twenty-minute delegation wave must not trip the
                // silence watchdog (its heartbeats already feed it, and this
                // covers the quiet tail after the last one).
                lastProgressMs.set(System.currentTimeMillis())
            } catch (t: Throwable) {
                if (stopped(token, callback)) {
                    stopRun()
                ...
                // An Error is not a fault to recover from. A VirtualMachineError
                // (out of memory, stack overflow), a ThreadDeath or any other
                // Error means the runtime itself is hurt or being torn down:
                // converting that into a "try a different approach" tool message
                // would loop a broken run and could corrupt state. Settle
                // everything the run owns — never leave the trail or a workflow
                // phase claiming to be RUNNING — report it, and let it propagate.
                if (t is Error) {
                    workflow?.failOpen(
                        clip(t.javaClass.simpleName + ": " + (t.message ?: ""), PHASE_CHARS)
                    )
                    settleAll(interrupted = true)
    ...
    /**
     * True asynchronous deadlines for a run's two time ceilings.
     *
     * The step-loop checks (wall-clock and no-output) only run BETWEEN steps,
     * so a single blocked streamChat, tool call, or sub-agent delegation could
     * sit past them indefinitely. The watchdog instead watches from a shared
     * daemon thread and cancels the run's token when a ceiling actually
     * expires — which unwinds the streaming IO thread, interrupts a wedged
     * tool worker, and stops a sub-agent, because every one of those layers
     * already observes the token.
     *
     * Only the top-level run arms one: a sub-agent shares its parent's token,
     * so the parent's watchdog already bounds it, and its own step-loop
     * checks are the inner defence.
     *
     * The ceiling that fired is recorded in [fired] BEFORE the token is
     * cancelled, so the run's stop path can end with that ceiling's marker
     * (which the Continue card recognises) instead of a bare stop. First
     * fire wins: once a ceiling is recorded the run is already unwinding, and
     * a second marker would only confuse the transcript.
     */
    private class RunWatchdog(
        private val token: CancellationToken,
        private val fired: java.util.concurrent.atomic.AtomicReference<String?>,
        private val wallClockAtMs: Long,
        private val noOutputMs: Long,
        private val lastProgressMs: java.util.concurrent.atomic.AtomicLong,
        private val wallClockReason: String,
        private val noOutputReason: String
    ) {
        private val lock = Any()
        private var checker: java.util.concurrent.ScheduledFuture<*>? = null
        private var closed = false

        init {
            // A 5 s cadence: the ceilings are 10 and 90 minutes, so a few
            // seconds of slop is nothing, and one wakeup per 5 s per active
            // run is negligible next to a streaming socket. The progress
            // timestamp is read, never written, here — the run's own paths
            // (streamed tokens, finished tools, sub-agent heartbeats) are the
            // writers, so nothing on the hot path pays for the watchdog.
            checker = SHARED.scheduleAtFixedRate({
                try {
                    tick()
                } catch (ignored: Throwable) {
                }
        ...
        private fun tick() {
            synchronized(lock) {
                if (closed || fired.get() != null) {
                    return
                }
            }
            val now = System.currentTimeMillis()
            if (now >= wallClockAtMs) {
                fire(wallClockReason)
            } else if (AgentBus.approvalParkedSince() <= 0L &&
                now - progressBaseline() >= noOutputMs
            ) {
                fire(noOutputReason)
            }
        }
        ...
        /**
         * B3: the no-output deadline is suspended while an approval is parked —
         * a user taking two minutes to answer is not a stalled run. When the last
         * ask settles, the answer counts as progress
         * ([AgentBus.lastApprovalSettledAt]), so the deadline resumes from the
         * answer instead of firing on the stale pre-ask timestamp. The
         * 90-minute wall clock above is never suspended: approval waiting is
         * idle, and idle still counts against the run's total budget.
         */
        private fun progressBaseline(): Long =
            Math.max(lastProgressMs.get(), AgentBus.lastApprovalSettledAt())

        private fun fire(reason: String) {
            val first = synchronized(lock) {
                !closed && fired.compareAndSet(null, reason)
            }
            if (first) {
                token.cancel()
            }
        ...
        /** Idempotent. Every terminal path of run() funnels through settleAll(). */
        fun close() {
            synchronized(lock) {
                if (closed) {
                    return
        ...
        companion object {
            private val SHARED: java.util.concurrent.ScheduledExecutorService =
                java.util.concurrent.Executors.newSingleThreadScheduledExecutor { work ->
                    Thread(work, "vega-run-watchdog").apply { isDaemon = true }
                }
        }
    }
    ...
    private fun thinkProse(reasoning: String, from: Int): String {
        ...
        return Think.visible(reasoning.substring(from)).replace(WHITESPACE_RUN, " ").trimJava()
    }
    ...
    private fun runApprovedDeletes(
        ...
        val paths = tools.pathListArg(args)
        if (paths.isEmpty()) {
        ...
        for (i in paths.indices) {
            ...
            try {
                // A session allowance covers the whole batch: re-asking per path
                // after the user already allowed the tool would re-ask an answered
                // question. ("Always allow" is only offered for reads today, so
                // this is consistency for now, not a behavior change.)
                val decision = if (isAllowedForSession(Tools.ToolNames.DELETE)) {
                    AgentBus.Decision(true, AgentBus.REASON_APPROVED)
                } else {
                    callback.requestApproval(Tools.ToolNames.DELETE, single)
                }
                if (!decision.approved) {
                    rejected++
                    details.append("REJECTED: ").append(path)
                    if (decision.reason != AgentBus.REASON_DECLINED) {
                        details.append(" (").append(decision.reason).append(")")
                    }
                    details.append('\n')
                    continue
                }
                ...
                Thread.currentThread().interrupt()
                details.append("CANCELLED: ").append(path).append(" (interrupted)\n")
                break
            } catch (e: Exception) {
                // B16: a throw must not discard the batch — the paths already
                // processed keep their details, and the throwing path is
                // recorded as failed rather than vanishing from the summary.
                failed++
                details.append("ERROR: ").append(path).append(": ")
                    .append(e.javaClass.simpleName).append(": ")
                    .append(e.message ?: "no detail").append('\n')
            }
        }
    ...
    private fun runSubAgents(
        ...
        if (batch != null) {
            // Extras beyond the cap and entries that are not objects are
            // COUNTED, not silently swallowed: a lead that sent seven tasks and
            // sees six run will otherwise never know one was dropped.
            droppedExtras = batch.length() - MAX_BATCH_AGENTS
            for (i in 0 until Math.min(batch.length(), MAX_BATCH_AGENTS)) {
                val entry = batch.optJSONObject(i)
                if (entry == null) {
                    malformedEntries++
                } else {
                    briefs.add(entry)
        ...
        // A brief that names verification IS the verification step: the gate
        // resets when the lead sends a verifier, not only when it re-reads the
        // changed files itself.
        if (depth == 0) {
            for (brief in briefs) {
                val label = (brief.optStr("name", "") + " " + brief.optStr("prompt", ""))
                    .lowercase(Locale.US)
        ...
        // The brief becomes the child's whole world. Cap it loudly rather than
        // letting a pasted dump eat the child's context in silence.
        for (brief in briefs) {
            clipBrief(brief)
        }
        ...
        if (briefs.size == 1) {
            ...
            // B19: startedAt is billed from the moment the worker actually
            // begins — which, on this path, is right here.
            phases[0]?.startedAt = System.currentTimeMillis()
        ...
        ) { runnable -> Thread(runnable, "vega-subagent").also { it.isDaemon = true } }
        ...
        // The registration is closed in the finally below. Without that every
        // `task` call leaked a listener onto the run's token, and a long session
        // accumulated one per delegation.
        val stopReg = token.onCancel(stopPool)
        // Declared outside the try: the post-wave sweep below the finally reads
        // waveFaults for rows that produced no report.
        val waveFaults = arrayOfNulls<Throwable>(briefs.size)
        try {
            // (brief index, future): a submit can be rejected while its siblings
            // succeed, so futures no longer line up with brief indices 1:1.
            val pending = ArrayList<Pair<Int, java.util.concurrent.Future<*>>>(briefs.size)
                ...
                try {
                    val future = pool.submit {
                        ...
                        try {
                            ...
                            // B12: a runtime Error is not a reportable outcome.
                            // Rethrow it — except while stopping, where a
                            // torn-down thread is a cancellation, not a crash —
                            // so it reaches the run's own Error path instead of
                            // becoming a polite "sub-agent failed" string.
                            if (error is Error &&
                                !token.isCancelled && !callback.isCancelled()
                            ) {
                                throw error
                            }
                            // A stop is not a failure: reporting it as ERROR
                            // taught the lead that the child had judged the work
                            // undoable, when it was simply never finished.
                            if (token.isCancelled || error is InterruptedException ||
                                error is java.util.concurrent.CancellationException
                            ) {
                                if (error is InterruptedException) {
                                    Thread.currentThread().interrupt()
                                }
                                "CANCELLED: sub-agent was stopped before it could report."
                            } else {
                                "ERROR: sub-agent failed: " +
                                    (error.message ?: error.javaClass.simpleName)
                            }
                        }
                            // The lead's timeout/stop notes race this write: whoever
                            // claims the slot first owns the outcome AND the
                            // single phase resolution — a worker that wakes up
                            // after the lead gave up must not overwrite the
                            // recorded note or resolve the row a second time.
                            val claimed = synchronized(results) {
                            ...
                            // Cleared even when the worker's Error escapes —
                            // a pooled thread must not keep a dead agent's name.
                            AgentBus.setAgentLabel(null)
                        }
                    }
                    pending.add(Pair(index, future))
                } catch (e: java.util.concurrent.RejectedExecutionException) {
                    // The pool was already shut down — a cancel raced the submit.
                    // Without this catch the brief vanished silently: no result,
                    // and a PENDING row that never resolves.
                    results[index] =
                        "CANCELLED: this sub-agent was never started; the run was stopped first."
                    settleUnstartedPhase(phases[index], true)
                }
            }
            // B1: wait in short slices, not one 30-minute get(). A single long
            // get() pinned the wave (and the run) until the child finished, no
            // matter what the token said — a stop waited out the slowest child.
            var waveError: Error? = null
            outer@ for ((index, future) in pending) {
                val deadline = System.currentTimeMillis() + WAVE_TIMEOUT_MS
                while (true) {
                    if (stopped(token, callback)) {
                        break
                    }
                    try {
                        future.get(WAVE_WAIT_SLICE_MS, java.util.concurrent.TimeUnit.MILLISECONDS)
                        break
                    } catch (e: java.util.concurrent.TimeoutException) {
                        if (System.currentTimeMillis() >= deadline) {
                            // A hung child must not pin the wave — and through it
                            // the run — forever. Interrupt it; if it wakes up its
                            // worker catch records the real outcome, and if it
                            // never does the note below stands as its report.
                            future.cancel(true)
                            val note = "ERROR: sub-agent timed out after " +
                                (WAVE_TIMEOUT_MS / 60000L) + " minutes and was " +
                                "interrupted before it could report."
                            // Same claim as the worker's: if the child reported
                            // in the gap between the deadline and the cancel,
                            // its real outcome stands and the note is dropped.
                            val claimed = synchronized(results) {
                                if (results[index] == null) {
                                    results[index] = note
                                    true
                            ...
                            if (claimed) {
                                resolvePhase(phases[index], note)
                            }
                            publish()
                            break
                        }
                        ...
                        // B11/B12: record the real cause. An Error escapes the
                        // wave to the run's own Error path; anything else reads
                        // as that child's failure below.
                        val cause = e.cause
                        if (cause is Error) {
                            waveError = cause
                            break@outer
                        }
                        waveFaults[index] = cause
                        break
                    } catch (e: InterruptedException) {
                        Thread.currentThread().interrupt()
                        break@outer
                    } catch (ignored: Throwable) {
                        // Whatever went wrong is already in `results` or will
                        // read as a missing report below; one child must not
                        // take down the wave.
                        break
                    }
                }
            ...
            if (waveError != null) {
                // B12: the child's Error propagates — cancel the rest of the
                // wave first so nothing is left running behind it.
            ...
            if (stopped(token, callback)) {
                // B1: a stop cancels every future that has not reported and
                // marks its row STOPPED — the wave no longer waits out the
                // timeout after the user pressed stop.
                for ((index, future) in pending) {
                    val note = "CANCELLED: sub-agent was stopped before it could report."
                    val claimed = synchronized(results) {
                        if (results[index] == null) {
                            try {
                                future.cancel(true)
        ...
        // Rows with no result. B11 distinguishes "never started" from
        // "started but left no report": only a row whose worker never began
        // (still PENDING) is settled as unstarted. A row that began but
        // produced nothing FAILED — with its real cause when one was recorded.
        // A row left PENDING keeps a promise on the board that nothing will
        // ever keep.
        for (i in briefs.indices) {
            // Claimed atomically: a worker still winding down from the
            // shutdownNow() above races this sweep, and the first writer wins.
            val unclaimed = synchronized(results) { results[i] == null }
            if (unclaimed) {
                val row = phases[i]
                if (row != null && row.status != WorkPhase.PENDING) {
                    val fault = waveFaults[i]
                    val note = if (fault != null) {
                        "ERROR: sub-agent failed before reporting: " +
                            fault.javaClass.simpleName + ": " + (fault.message ?: "no detail")
                    } else {
                        "ERROR: sub-agent started but produced no report."
                    }
                    val claimed = synchronized(results) {
                        if (results[i] == null) {
                            results[i] = note
                    ...
                    if (claimed) {
                        resolvePhase(row, note)
                    }
                } else {
                    settleUnstartedPhase(row, token.isCancelled || callback.isCancelled())
                }
            }
        }
        ...
        // Every report is collected, labelled, and budgeted: three verbose
        // children can otherwise hand the lead 30 KB of undifferentiated text
        // and eat the context window the wave was built to protect.
        val share = MAX_WAVE_RESULT_CHARS / briefs.size.coerceAtLeast(1)
        val sb = StringBuilder()
        for (i in briefs.indices) {
            if (sb.isNotEmpty()) {
                sb.append("\n\n")
            }
            val agentNo = phases[i]?.agentId ?: (i + 1)
            sb.append("--- ").append(Fa.WF_AGENT.format(agentNo))
            val topic = phases[i]?.topic.orEmpty()
            if (topic.isNotBlankJava()) {
                sb.append(": ").append(clip(topic, 80))
            }
            sb.append(" ---\n")
            sb.append(budgetedReport(results[i], share, token, callback))
        }
        if (droppedExtras > 0 || malformedEntries > 0) {
            sb.append("\n\nNOTE: ")
            if (droppedExtras > 0) {
                sb.append(droppedExtras).append(" extra task")
                if (droppedExtras != 1) {
                    sb.append("s")
                }
                sb.append(" ignored (at most ").append(MAX_BATCH_AGENTS)
                    .append(" per call)")
            ...
            if (malformedEntries > 0) {
                sb.append(malformedEntries).append(" malformed entr")
                sb.append(if (malformedEntries == 1) "y" else "ies")
                sb.append(" skipped (not { name, prompt, phase } objects)")
            }
            sb.append(".")
        }
        return sb.toString()
    ...
    /**
     * Caps a sub-agent brief at [MAX_BRIEF_CHARS], marking the cut.
     *
     * The cut is marked because a silent truncation is worse than a loud one:
     * the lead would believe the child saw details it never received, and the
     * child's confusion would read as incompetence rather than a missing input.
     */
    private fun clipBrief(brief: JSONObject) {
        val prompt = brief.optStr("prompt", "")
        if (prompt.length > MAX_BRIEF_CHARS) {
            brief.put(
                "prompt",
                prompt.substring(0, MAX_BRIEF_CHARS) +
                    "\n\n[brief truncated: the original was " + prompt.length +
                    " characters; only the first " + MAX_BRIEF_CHARS + " were kept]"
            )
        }
    }

    /**
     * Closes a board row whose sub-agent never started — a rejected submit, or
     * a wave that ended before the pool picked it up. A row left PENDING is a
     * promise the board keeps showing; a closed row with a note is the truth.
     */
    private fun settleUnstartedPhase(phase: WorkPhase?, cancelled: Boolean) {
        val row = phase ?: return
        if (row.status != WorkPhase.PENDING) {
            return
        }
        row.status = if (cancelled) WorkPhase.STOPPED else WorkPhase.FAILED
        row.note = if (cancelled) "Stopped before it started." else "Was never started."
        row.endedAt = System.currentTimeMillis()
    }

    /**
     * One sub-agent's report, clipped to its share of [MAX_WAVE_RESULT_CHARS].
     *
     * A missing report is classified, not just lamented: after a stop it is a
     * cancellation, otherwise it is a failure. The lead acts differently on
     * each — retrying a cancelled child is pointless, retrying a failed one
     * may not be.
     */
    private fun budgetedReport(
        report: String?,
        share: Int,
        token: CancellationToken,
        callback: Callback
    ): String {
        val body = report ?: if (token.isCancelled || callback.isCancelled()) {
            "CANCELLED: sub-agent produced no report; the run was stopped."
        } else {
            "ERROR: sub-agent produced no report."
        }
        return if (body.length > share) {
            body.substring(0, share) + "\n[report truncated to " + share + " characters]"
        } else {
            body
        }
    }
    ...
    /**
     * What the model hears when an approval is not granted. A bare "user
     * declined" for a timeout or a cancellation taught the model the user had
     * judged the action and said no — so it argued, rephrased, or retried.
     * Each outcome gets its own words: a timeout is not a verdict, and a
     * cancellation is not a rejection.
     */
    private fun approvalRefusalText(reason: String): String = when (reason) {
        AgentBus.REASON_TIMED_OUT ->
            "NOT ANSWERED: the approval request timed out with no answer — treat " +
                "it as declined for now. Do NOT silently retry the same action. " +
                "Tell the user what you were waiting to do and ask how they'd " +
                "like to proceed."
        AgentBus.REASON_CANCELLED, AgentBus.REASON_STOPPED, AgentBus.REASON_RUN_ENDED ->
            "CANCELLED: the run was stopped while waiting for approval. Do not " +
                "retry anything; end your turn."
        else ->
            "REJECTED: The user declined this action. Do not retry it. Ask how " +
                "they'd like to proceed or continue with other steps."
    }
    ...
    private fun looksLikeCallOpening(body: CharSequence): Boolean {
        // Allocation-free: this runs on every streamed token, and the old
        // toString() copied the whole body each time.
        val fence = indexOf(body, "```json")
        if (fence < 0) {
            return false
        }
        val key = indexOf(body, "\"tool\"", fence)
        return key >= 0 && key - fence <= CALL_OPENING_WINDOW
    }

    /**
     * indexOf over a CharSequence without materialising it. StringBuilder has
     * its own; the fallback below keeps generic CharSequence callers
     * allocation-free.
     */
    private fun indexOf(haystack: CharSequence, needle: String, from: Int = 0): Int {
    ...
    private fun resolvePhase(phase: WorkPhase?, result: String) {
        val row = phase ?: return
        val done = !result.startsWith("CANCELLED") && !result.startsWith("ERROR")
        row.status = when {
        ...
        row.endedAt = System.currentTimeMillis()
        // Verification gate: a phase that closed DONE without any verification
        // since arms the nudge every 3 closes (it re-arms after the next
        // verification). resolvePhase also runs on sub-agent pool threads, so
        // the counter is guarded — the run thread reads it between waves.
        if (done) {
            synchronized(this) {
                unverifiedCloses++
                if (unverifiedCloses >= MAX_UNVERIFIED_CLOSES) {
                    // Restart the count at every arming: the model may ignore
                    // a nudge, and the gate must nag again after 3 more
                    // unverified closes rather than disarming itself forever.
                    unverifiedCloses = 0
                    verificationNudgeDue = true
                }
            }
        }
    }
    ...
    /**
     * Verification gate: verification was observed (a re-read of changed work
     * or a verifier sub-agent) — restart the count AND disarm a nudge that was
     * armed but not yet delivered, which the verification made stale.
     */
    private fun noteVerified() {
        synchronized(this) {
            unverifiedCloses = 0
            verificationNudgeDue = false
        }
    ...
    private fun runSubAgent(
        ...
        // B10: the sub-agent's FINAL message, explicitly tracked — not "the last
        // non-empty prose". onStepFinalized fires once per finished step; the
        // last firing is the report, even when it is empty (an empty final
        // message is a missing report, not a reason to resurrect older prose).
        var finalVisible: String? = null
        ...
        var steps = 0
        // B2: the child's tokens and thinking feed the PARENT's no-output
        // deadline through the publish hook (publishWithProgress), throttled so
        // a fast child does not republish the board on every token.
        var lastHeartbeat = 0L
        fun heartbeat() {
            val now = System.currentTimeMillis()
            if (now - lastHeartbeat >= CHILD_HEARTBEAT_MS) {
                lastHeartbeat = now
                publish()
            }
        }
        ...
        val childCallback = object : Callback {
            ...
            override fun onDelta(message: Message, delta: String) {
                // B2: the child's output proves the delegation is alive — the
                // parent's no-output deadline must not fire mid-stream.
                heartbeat()
            }
            override fun onNewAssistantMessage(message: Message) {}
            override fun onThinking(message: Message, thinking: String) {
                // B2: thinking counts as activity too.
                heartbeat()
            }
            ...
            override fun onStepFinalized(message: Message) {
                // B10: every step overwrites — the value left standing when the
                // run ends is the final message, whatever it holds.
                finalVisible = stripToolCalls(Think.visible(message.content)).trimJava()
            }
            ...
            override fun requestApproval(tool: String, args2: JSONObject?): AgentBus.Decision =
        ...
        try {
            ...
            // B12: an Error is the runtime failing, not a reportable outcome —
            // let it reach the run's own Error path instead of becoming a
            // polite "sub-agent failed" string.
            if (error is Error) {
                throw error
            }
        ...
        val text = (finalVisible ?: "").trimJava()
        if (text.isEmpty()) {
            // B10: ERROR-prefixed, so the board marks the row FAILED — an empty
            // final message is a missing report, and "no report" is a failure,
            // not a completion.
            return "ERROR: sub-agent finished without a usable final report. Do this step yourself."
        }
    ...
    private fun buildApiMessages(
        ...
        val history: List<Message> = synchronized(chat.messages) { chat.messages.toList() }
        val startIndex = compactHistoryStart(history, historyChars)
        if (startIndex > 0) {
            // Summary compaction (Claude Code's compaction trick): before the
            // prefix is dropped, a one-turn no-tool call summarises it into the
            // 9-section template. When the summary call fails, the old
            // subtractive behaviour is the fallback — the run never depends on
            // the summary to continue.
            val summary = compactionSummary(history, startIndex, token)
            out.put(
                JSONObject().put("role", "system").put(
                    "content",
                    if (summary != null) {
                        "Earlier conversation content was compacted to keep this request within provider context limits. The summary below is authoritative for everything that was dropped; the retained recent messages that follow are authoritative for what came after.\n\n" +
                            summary
                    } else {
    ...
    /**
     * B5: the system prompt used to be rebuilt from scratch twice per run path
     * (once for the context budget, once per request) — and every build walks
     * the skills directory and re-reads memory. It is now built once and cached
     * for the run, keyed on everything that can change it: the mode, the
     * (boosted) thinking level, the skills directory state, the memory file
     * state, a snapshot of the prompt-affecting prefs, and the plan file path.
     * A changed skill, a fresh memory fact or a flipped setting changes the
     * key and rebuilds the prompt; nothing else does.
     */
    private fun systemPrompt(): String {
        val key = systemPromptKey()
        val cached = cachedSystemPrompt
        if (cached != null && key == cachedSystemPromptKey) {
            return cached
        }
        val built = buildSystemPrompt()
        cachedSystemPrompt = built
        cachedSystemPromptKey = key
        return built
    }

    /**
     * Cache key for [systemPrompt]: every input the prompt is built from.
     */
    private fun systemPromptKey(): String {
        val skillsStamp = try {
            dirStamp(Skills.skillsRoot(context, prefs))
        } catch (e: Exception) {
            0L
        }
        val memoryStamp = try {
            val memoryFile = File(context.filesDir, MEMORY_FILE_NAME)
            if (memoryFile.exists()) memoryFile.lastModified() else -1L
        } catch (e: Exception) {
            -1L
        }
        return prefs.mode() + "|" + boostedThinkingLevel() + "|" +
            skillsStamp + "|" + memoryStamp + "|" + prefsSnapshot() + "|" +
            (activePlanFile?.absolutePath ?: "-")
    }

    /**
     * Newest mtime in a directory TREE. Skills live two levels down
     * (`<root>/<skill>/SKILL.md`), and editing one changes the file's mtime
     * but not its parents' — so a top-level-only scan missed exactly the edit
     * the cache exists to notice. Bounded at [MAX_STAMP_FILES]: a skills folder
     * is small, but a runaway symlink or a dumped archive must not turn a
     * cache key into an unbounded tree walk.
     */
    private fun dirStamp(dir: File): Long {
        ...
        stack.add(dir)
        var seen = 0
        while (stack.isNotEmpty() && seen < MAX_STAMP_FILES) {
            ...
            val kids = try {
                if (f.isDirectory) f.listFiles() else null
            } catch (e: Exception) {
                null
            }
            if (kids != null) {
    ...
    /** The prompt-affecting prefs, folded into one int for the cache key. */
    private fun prefsSnapshot(): Int {
        var h = prefs.mode().hashCode()
        h = 31 * h + prefs.maxTokens()
        h = 31 * h + prefs.systemPrompt().hashCode()
        h = 31 * h + (if (prefs.webSearch()) 1 else 0)
        h = 31 * h + (if (prefs.dynamicWorkflow()) 1 else 0)
        h = 31 * h + (try {
            prefs.skillsFolder()
        } catch (e: Exception) {
            ""
    ...
    private fun buildSystemPrompt(): String {
        ...
        sb.append("Your name is Vega Agent. When you speak Persian, always call yourself «وگا ایجنت» (Vega Agent); when you speak English, call yourself Vega Agent. ")
        ...
        sb.append(
            "- You have a budget of " + maxStepsForLevel(prefs.effectiveThinkingLevel()) +
                " tool steps for this run (reasoning effort " +
                prefs.effectiveThinkingLevel().uppercase() +
                ") — pace yourself: investigate efficiently, batch independent calls, and keep enough budget to verify the final result.\n"
        )
        sb.append("- Work step by step and use each tool result to decide the next step. Prefer verifying (read before you edit, check a link before you download) over guessing. Batch INDEPENDENT tool calls in one message (see \"Tool use\" below) — but never batch calls where one needs another's result.\n")
        ...
        sb.append("\n\n")
        if (Prefs.MODE_PLAN == mode) {
            // The one writable path in PLAN mode — surfaced here so the model
            // never has to guess where the plan goes.
            sb.append("PLAN FILE: ")
            sb.append(
                try {
                    activePlanFile?.absolutePath
                    ...
                    null
                } ?: "<workspace>/.vega/plans/<this-chat>.md"
            )
            sb.append(
                " — this is the ONLY file you may write in PLAN mode. " +
                    "Write the finished numbered plan there with write_file (new plan) " +
                    "or edit_file (revising an existing one). Its folder already " +
                    "exists, so no make_dir is needed. Every other write is refused.\n\n"
            )
        }
        ...
        // Installed agent skills: the model sees the catalog and picks only
        // the skills that genuinely fit the current task. The block is empty
        // (and nothing is injected) when no skills are installed.
        val skillsBlock = Skills.promptBlock(context, prefs)
        if (skillsBlock.isNotBlankJava()) {
            sb.append(skillsBlock)
            sb.append("\n")
        }
        sb.append("# Tool use — STRICT format\nTo call tools, output fenced ```json code blocks, each holding ONE JSON object with \"tool\" and \"args\". Emit SEVERAL blocks in ONE message when the calls are INDEPENDENT (no call may need another's result): read-only calls (read_file, list_dir, file_info, search_files, glob, web_search, web_fetch, web_fetch_many, read_pdf, recall) run in parallel, the rest run serially in the order you wrote them, and each result comes back separately as [TOOL RESULT: <name>]. After the results the app calls you again — so you keep going turn after turn until the job is finished.\nHARD RULES (breaking these silently aborts the run):\n1. Each block's JSON must be valid: no comments, no // notes, no trailing text after the closing brace, no markdown inside it.\n2. Put NOTHING after the closing ``` of the LAST tool block. Say any short preamble BEFORE the first block.\n3. URLs and paths must contain NO spaces. Never write a domain like \"site. com\"; write \"site.com\". Percent-encode real spaces as %20.\n4. Only batch INDEPENDENT calls. If call B needs call A's result (a path, an id, a value you have not seen yet), emit A alone, wait for its result, then emit B.\n\n# Keep going until done (critical)\n- After a [TOOL RESULT], DECIDE THE NEXT STEP and act — do NOT end your turn just because one tool finished. The loop continues automatically.\n- Only produce a final answer with NO json block when the WHOLE task is actually complete (e.g. the file is downloaded and you've confirmed it). A final answer ends the run.\n- Never reply with an empty or filler message. If you found a verified download link ([OK ✓]) in a previous result, your very next turn MUST be a download_file call with that exact link — do not re-search, re-open the page, or stop.\n- If you are ever unsure what to do next, take the most reasonable next action rather than stopping.\n\n")
        sb.append("Example:\n```json\n{\"tool\": \"read_file\", \"args\": {\"path\": \"")
        sb.append(workspaceRoot)
        sb.append("/Download/notes.txt\"}}\n```\n\n")
        sb.append("# Available tools\n")
        sb.append("Paths: send exactly ONE path per call (a single \"path\"). If you catch yourself writing both \"path\" and \"filename\"/\"file_path\", put the full file path in \"path\" alone. Absolute paths and workspace-relative paths both work.\n")
        ...
        sb.append("- delete_path { path | paths: [path, ...] } — delete one file/folder or a list of paths. \"paths\" also accepts a single string, and \"path\" accepts the aliases file_path/file/filename/target/dir. In ACCEPT mode, every path in a list is approved and executed separately. (modifying)\n")
        sb.append("- make_dir { path } — create a directory. (modifying)\n")
        sb.append("- move_path { from, to, overwrite? } — move/rename. Refuses to replace an existing destination unless overwrite:true. (modifying)\n")
        ...
        sb.append("- download_file { url, filename?, referer?, sha256? } — download a file from the internet straight into the phone's Downloads folder (music, images, videos, documents, any file). Shows live progress and reports the saved path. Pass sha256 when the release page publishes one: the file is verified before it lands in Downloads and a mismatch fails loudly instead of saving a bad file. (modifying)\n")
        if (prefs.webSearch()) {
            sb.append("- web_search { query } — search the web.\n")
            sb.append("- web_fetch { url } — fetch a web page as readable text.\n")
            sb.append("- web_fetch_many { urls } — read SEVERAL pages at once, in parallel, when you already know the URLs (e.g. after a web_search). urls is an array of up to 8 page URLs; returns one digest per page.\n")
        }
        sb.append("- install_skill { name, description, instructions, source_url?, allowed_tools? } — install an agent skill into the app's skills folder. Use it when the user asks you to FIND skills for a topic on the web and install them. (modifying)\n")
        if (prefs.dynamicWorkflow() && depth == 0 && Prefs.MODE_PLAN != mode) {
        ...
        sb.append("- remember { text } — save a durable fact to long-term memory.\n")
        ...
        sb.append("- For multi-step tasks, work step by step — one tool per turn, unless the calls are independent (then batch them in one message).\n")
        if (prefs.webSearch()) {
            sb.append("- Use web_search for current information, then web_fetch to read promising results.\n")
            sb.append("# Finding and installing skills from chat\n")
            sb.append("- When the user asks you to FIND skills for a topic and install them (e.g. \"find a few skills for X and install them\"), do it yourself: web_search for the topic plus \"agent skill SKILL.md github\", web_fetch the most promising SKILL.md files (prefer raw.githubusercontent.com URLs), then call install_skill once per skill with the FAITHFUL name, description and instructions from what you fetched.\n")
            sb.append("- name: short kebab-case (lowercase, digits, hyphens). description: one line, specific and action-oriented — it is the WHEN clause the future you reads to decide whether the skill fits. instructions: the complete skill body in markdown, self-contained.\n")
            sb.append("- source_url: the page or repo you took it from. allowed_tools: optional comma-separated tool names the skill works best with.\n")
            sb.append("- NEVER invent a skill's content: if no usable SKILL.md turns up, say so and install nothing. Install at most a handful per request, and report each installed name in one short line.\n")
    ...
    // ---- plan mode: session plan file, reminders, approved-plan handoff ----

    /**
     * The session plan file: `<workspace>/.vega/plans/<chatId>.md`. In PLAN
     * mode it is the ONLY writable path; in ACCEPT mode it carries the
     * approved plan across the mode flip. The chat id is sanitised to a safe
     * filename — it is app-generated, but a plan path must never be able to
     * escape its directory.
     */
    private fun sessionPlanFile(chatId: String?): File? {
        ...
        return try {
            File(File(Tools.externalRoot(context), ".vega/plans"), safe.toString() + ".md")
        } catch (e: Exception) {
            null
        }
    }
    ...
    /**
     * True when this call writes exactly the session plan file. The
     * containment check mirrors Tools.resolve's (canonical root + separator
     * prefix) but is reimplemented locally — small enough to own, and this
     * file may not take on new behaviour from Tools.
     */
    private fun isPlanFileWrite(call: ToolCall): Boolean {
        if (Tools.ToolNames.WRITE_FILE != call.name &&
            Tools.ToolNames.EDIT_FILE != call.name
        ) {
            return false
        }
        val planFile = activePlanFile ?: return false
        ...
        return try {
            resolveUnderRoot(raw).canonicalPath == planFile.canonicalPath
        } catch (e: Exception) {
            // Unresolvable or escaping paths fail closed: not the plan file,
            // so the plan guard still refuses them.
            false
        }
    }

    /**
     * Resolves a tool-supplied path against the workspace root, refusing
     * anything that escapes it — the small containment core of Tools.resolve.
     */
    private fun resolveUnderRoot(path: String): File {
        val root = Tools.externalRoot(context)
        var trimmed = path.trimJava()
        if (trimmed.startsWith("~")) {
            trimmed = root.absolutePath + trimmed.substring(1)
        }
        val requested = File(trimmed)
        val file = if (requested.isAbsolute) requested else File(root, trimmed)
        val rootPath = root.canonicalPath
        val candidate = file.canonicalPath
        if (candidate != rootPath && !candidate.startsWith(rootPath + File.separator)) {
            throw SecurityException("path is outside the workspace")
        }
        return file
    }

    /**
     * Reads the approved plan for the handoff, or null when there is nothing
     * to hand off: no plan file, an empty one, or one already consumed. The
     * `.consumed` sidecar records the plan file's mtime at consumption, so
     * editing the plan after approval re-arms the handoff.
     */
    private fun readApprovedPlan(plan: File): String? {
        return try {
            if (!plan.isFile) {
                return null
            }
            val marker = File(plan.parentFile, plan.name + ".consumed")
            val stamp = plan.lastModified().toString()
            if (marker.isFile && marker.readText().trimJava() == stamp) {
                return null
            }
            val text = plan.readText(Charsets.UTF_8)
            if (text.isBlankJava()) {
                return null
            }
            Util.truncate(text, SUMMARY_OUTPUT_CHARS)
        } catch (e: Exception) {
            null
        }
    }

    /** Marks the plan consumed so the handoff fires exactly once per approval. */
    private fun markPlanConsumed(plan: File) {
        try {
            File(plan.parentFile, plan.name + ".consumed")
                .writeText(plan.lastModified().toString(), Charsets.UTF_8)
        } catch (ignored: Exception) {
        }
    }

    /**
     * The approved-plan handoff (ExitPlanMode semantics): the plan sheet flips
     * the stored mode to ACCEPT when the user taps "Run plan", and the next
     * run starts from the approved plan instead of asking what to do.
     */
    private fun approvedPlanHandoff(plan: File, approved: String): String =
        "<system-reminder>User has approved your plan. Start with updating " +
            "your todo list. Your plan was saved to: " + plan.absolutePath +
            ". Follow it step by step.\n\n## Approved Plan:\n" + approved +
            "\n</system-reminder>"

    /**
     * Plan mode's per-turn workflow driver, injected as a <system-reminder>
     * user message (Claude Code's attachment trick): the full 5-phase
     * workflow every 5th turn, a one-line sparse reminder between turns. A
     * system-reminder never busts the cached system prompt and stays
     * cache-friendly between turns.
     */
    private fun planModeReminder(turn: Int): String {
        val path = try {
            activePlanFile?.absolutePath
        } catch (e: Exception) {
            null
        ...
        if (turn % 5 != 1) {
            return "<system-reminder>Plan mode: read-only. Keep investigating; " +
                "write the numbered plan to " + path +
                " (the only writable file) when ready.</system-reminder>"
        }
        return "<system-reminder>\n" +
            "PLAN MODE — read-only investigation. Work these 5 phases:\n" +
            "1. EXPLORE: investigate with read-only tools (read_file, search_files, list_dir, web_search, web_fetch). Launch parallel reads for independent questions.\n" +
            "2. DESIGN: decide the concrete approach and the exact changes each step needs.\n" +
            "3. REVIEW: re-read the critical files your investigation identified.\n" +
            "4. WRITE THE PLAN: save the final numbered plan to " + path + " with write_file (new) or edit_file (revise) — the ONLY file you may write in this mode.\n" +
            "5. PRESENT: end your turn with the numbered plan so the user can approve it; do not ask for permission in text.\n" +
            "The task tool is not available in PLAN mode. The remember tool still works.\n" +
            "</system-reminder>"
    }

    /** The last user message's text — the thinking-keyword boost reads this. */
    private fun lastUserText(chat: Chat): String {
        val messages = synchronized(chat.messages) { chat.messages.toList() }
        for (i in messages.indices.reversed()) {
            val message = messages[i]
    ...
    /**
     * The 9-section template for the one-turn compaction summary (Claude
     * Code's compaction contract). The summary replaces the dropped prefix;
     * it must be dense and self-contained because the model never sees the
     * original again.
     */
    private fun compactSummaryPrompt(): String =
        "You are compacting a conversation for an AI agent that will continue the task. " +
            "Write a dense, structured summary of the conversation below using EXACTLY these 9 sections:\n" +
            "1. Primary Request and Intent\n" +
            "2. Key Technical Concepts\n" +
            "3. Files and Code Sections\n" +
            "4. Errors and Fixes\n" +
            "5. Problem Solving\n" +
            "6. All User Messages (quote every user message word for word)\n" +
            "7. Pending Tasks\n" +
            "8. Current Work\n" +
            "9. Next Step\n" +
            "Be concrete: names, paths, values, decisions. No preamble, no advice — just the summary."

    /**
     * Summarises the dropped history prefix with a single no-tool LLM call,
     * so compaction keeps meaning instead of just deleting it. Cached per
     * run: only re-summarises when the dropped prefix grew past what the
     * cached summary describes. Returns null on any failure — the caller
     * falls back to the old subtractive notice and the run continues.
     */
    private fun compactionSummary(
        history: List<Message>,
        startIndex: Int,
        token: CancellationToken
    ): String? {
        if (startIndex <= compactSummaryStart && compactSummary != null) {
        ...
        for (i in 0 until startIndex) {
            // Strictly under budget: the last message is cut to the REMAINING
            // budget, not appended whole past it — the old check-then-append
            // could overshoot by a full truncated message.
            val remaining = SUMMARY_INPUT_CHARS - used
            if (remaining <= 0) {
                break
            }
            val message = history[i]
            ...
            // The header is part of the input too: budget it before the text,
            // or a long prefix overshoots by one header per message.
            val header = "\n\n[" + role + "]\n"
            val remainingForText = remaining - header.length
            if (remainingForText <= 0) {
                break
            }
            val text = Util.truncate(
                Think.stripForModel(message.content), Math.min(2000, remainingForText)
            )
            if (text.isBlankJava()) {
                continue
            }
            input.append(header).append(text)
        ...
        request.put(
            JSONObject().put("role", "system").put("content", compactSummaryPrompt())
        )
        request.put(
            JSONObject().put("role", "user")
                .put("content", "Summarise this conversation prefix:" + input.toString())
        )
        val output = StringBuilder()
        var failed = false
        val summaryCallback = object : LlmClient.StreamCallback {
            override fun onToken(text: String) {
                output.append(text)
            }
        ...
        try {
            llm.streamChat(request, token, summaryCallback)
        } catch (e: Exception) {
            failed = true
        }
        ...
        val summary = output.toString().trimJava()
        // Too thin to trust as the authoritative record of the prefix.
        if (summary.length < 40) {
            return null
        }
        compactSummary = Util.truncate(summary, SUMMARY_OUTPUT_CHARS)
        compactSummaryStart = startIndex
        return compactSummary
    }
    ...
    private fun reasoningBlock(): String {
        ...
        val level = boostedThinkingLevel()
        return "# Reasoning effort: " + level.uppercase() + "\n" +
            reasoningGuidance(level) + "\n" + reasoningIntegrityRule()
    }

    /**
     * The thinking level for this run: the configured level, raised — never
     * lowered — by magic keywords in the user's message ("ultrathink", "think
     * harder", "think", ...). The boost is computed once at run start from the
     * last user message.
     */
    private fun boostedThinkingLevel(): String {
        val base = prefs.effectiveThinkingLevel()
        val boost = thinkingBoost ?: return base
        return if (thinkingRank(boost) > thinkingRank(base)) boost else base
    }

    companion object {
        /**
         * The run's turn budget, scaled by reasoning effort — this is what
         * makes the low…max dial REAL rather than a label: a bigger budget
         * means the agent may keep investigating, retrying and verifying
         * instead of being cut off mid-task.
         */
        internal fun maxStepsForLevel(level: String?): Int = when (level) {
            "low" -> 40
            "medium" -> 80
            "high" -> 120
            "xhigh" -> 160
        ...
        /** Per-sub-agent step ceiling, scaled the same way. */
        internal fun maxSubagentStepsForLevel(level: String?): Int = when (level) {
            "low" -> 16
            "medium" -> 24
            "high" -> 32
            "xhigh" -> 40
        ...
        /**
         * B9: the task tool is refused outright in PLAN mode. A delegation
         * would inherit the read-only mode and refuse every change, so it
         * buys nothing — the model investigates itself instead.
         */
        private const val TASK_PLAN_REFUSAL =
            "BLOCKED: the task tool is not available in PLAN mode — a sub-agent would inherit this read-only mode and refuse every change. Nothing was delegated.\n" +
                "Do NOT retry the delegation. Investigate with read-only tools yourself (read_file, search_files, list_dir, web_search, web_fetch) and write the numbered plan to the session plan file under .vega/plans/ — the one file you may write in this mode. Then stop and wait for the user to approve it."
        ...
        /**
         * How long the wave waits for one sub-agent before interrupting it.
         *
         * A sub-agent is already bounded by its effort-scaled step ceiling, so a
         * healthy child always finishes far inside this. The ceiling exists for
         * the unhealthy one: a thread parked in a socket read that swallowed
         * its interrupt, a retry loop that never notices the cancel. Without a
         * ceiling the lead waits on `Future.get()` forever and the whole run
         * hangs with it.
         */
        private const val WAVE_TIMEOUT_MS = 30L * 60L * 1000L

        /**
         * Longest brief a sub-agent receives, in characters.
         *
         * The brief becomes the child's whole world — its system prompt plus
         * this — so a 40 KB dump the lead pasted in would eat the child's
         * context before it did any work. Truncated, loudly: a silent cut would
         * send the child off missing the very details the lead thought it had.
         */
        private const val MAX_BRIEF_CHARS = 6000

        /**
         * Budget for the concatenated sub-agent reports handed back to the lead.
         *
         * Three verbose children can easily return 30 KB of report; feeding all
         * of it to the lead every wave is how a long task eats the very context
         * window the wave was built to protect. Each report is clipped to its
         * share, and the cut is marked so the lead knows there was more.
         */
        private const val MAX_WAVE_RESULT_CHARS = 12000
         ...
         *
         * B8: per-chat, not global, and consumed on read. The old implementation
         * reset this bit at the start of EVERY run, so a background run for
         * chat B silently cleared the signal a foreground run for chat A had
         * just set — and the plan sheet for chat A never opened. The flag now
         * lives in a per-chat map keyed by [AgentBus.activeChatId], and reading
         * it clears it: the read IS the consume, so the sheet fires exactly
         * once per refusal. This matters because MainActivity reads this
         * property directly at run completion and has no separate consume call
         * — a pure (non-consuming) getter here would re-fire the sheet after
         * every later run in the chat until something else cleared it, and a
         * run-start reset would erase refusals the UI had not seen yet. The
         * getter resolves the current (or most recent, after AgentBus.finish()
         * clears the active chat) chat, exactly like [consumeWantedChanges].
         */
        var lastRunWantedChanges: Boolean
            get() = consumeWantedChanges()
            private set(value) = synchronized(wantedChangesLock) {
                val id = AgentBus.activeChatId ?: lastWantedChangesChat ?: return
                lastWantedChangesChat = id
                if (value) {
                    wantedChangesByChat[id] = true
                } else {
                    wantedChangesByChat.remove(id)
                }
            }
        ...
        /**
         * B8: atomically reads and clears the plan-sheet signal for the current
         * (or most recent) chat. Returns true when a run asked for a refused
         * change since the last consume — the plan sheet opens on true and stays
         * shut on false, exactly once per refusal. Safe to call from any thread.
         */
        fun consumeWantedChanges(): Boolean = synchronized(wantedChangesLock) {
            val id = AgentBus.activeChatId ?: lastWantedChangesChat ?: return false
            return wantedChangesByChat.remove(id) == true
        }

        /** Guard for [wantedChangesByChat] and [lastWantedChangesChat]. */
        private val wantedChangesLock = Any()

        /** Per-chat backing for [lastRunWantedChanges]; see the B8 note there. */
        private val wantedChangesByChat = HashMap<String, Boolean>()

        /** Most recent chat that touched the wanted-changes signal. */
        @Volatile
        private var lastWantedChangesChat: String? = null
        ...
        /**
         * Absolute ceiling for one run's wall-clock time.
         *
         * The step and fault budgets bound WORK; this bounds TIME. Ninety
         * minutes is far beyond any healthy run — a run this old is stuck, not
         * working, and used to hang until the user pressed stop.
         */
        private const val RUN_WALL_CLOCK_MS = 90L * 60L * 1000L

        /**
         * Silence ceiling: no token, no finished tool call and no sub-agent
         * heartbeat for this long ends the run. Ten minutes matches the
         * approval timeout's scale — a provider quiet that long is not coming
         * back on its own.
         */
        private const val NO_OUTPUT_MS = 10L * 60L * 1000L

        /** Floor for the history budget: the overflow fast path never compacts below this. */
        private const val MIN_HISTORY_CHARS = 20000

        /** How many times the overflow fast path may halve the history budget. */
        private const val MAX_OVERFLOW_SHRINKS = 3

        /**
         * B17: how many times a stalled step may be recovered before the run is
         * declared stuck. Named so the ceiling reads as policy, not a literal.
         */
        private const val MAX_STALL_RECOVERIES = 6

        /**
         * B4: tokens between message.content synchronisations while streaming.
         * The final content is always synced exactly; this only throttles the
         * intermediate copies, which were O(n²) on long answers.
         */
        private const val STREAM_SYNC_EVERY = 25

        /**
         * Upper bound for the [dirStamp] tree walk. Skills folders are tens of
         * files; this is two orders of magnitude of headroom before a symlink
         * cycle or a stray archive can turn a cache key expensive.
         */
        private const val MAX_STAMP_FILES = 512

        /**
         * Whitespace runs in reasoning prose. Compiled once: thinkProse runs on
         * every throttled reasoning publish, and compiling a pattern per call
         * was pure waste.
         */
        private val WHITESPACE_RUN = Regex("\\s+")

        /**
         * B2: sub-agent heartbeats feed the parent's no-output deadline at most
         * this often. 30s is far inside the 10-minute ceiling and keeps a
         * quiet child from ever looking stalled.
         */
        private const val CHILD_HEARTBEAT_MS = 30_000L

        /**
         * B1: the wave wait checks each sub-agent future in slices of this
         * length, so a stop is honoured within ~2s instead of after the full
         * 30-minute wave timeout.
         */
        private const val WAVE_WAIT_SLICE_MS = 2000L

        /**
         * Summary-compaction budget: how much of the dropped prefix the
         * one-turn summariser may read, and how long its output may be.
         */
        private const val SUMMARY_INPUT_CHARS = 12000
        private const val SUMMARY_OUTPUT_CHARS = 6000

        /** Number of phases that may close unverified before the gate nudges. */
        private const val MAX_UNVERIFIED_CLOSES = 3

        /**
         * True when the provider rejected the request for being too large.
         * Matched against the provider's own error detail (carried in the
         * stream error text), never against model output.
         */
        internal fun isContextOverflow(error: String): Boolean {
            val e = error.lowercase(Locale.US)
            return e.contains("context_length_exceeded") ||
                e.contains("maximum context length") ||
                e.contains("max context length") ||
        ...
        /**
         * Widening pause between step-level fault recoveries. A fixed ladder:
         * the widening shape is a contract (see
         * testFaultBackoffWidensAndIsBounded), and the randomisation lives in
         * [faultSleepMs], applied where the sleep actually happens.
         */
        ...
        /**
         * The actual sleep before a fault recovery: the ladder above, plus
         * ±25% jitter.
         *
         * Without it every client that hit the same outage retried in
         * lockstep, re-hammering the provider at exactly the same instants —
         * the classic thundering herd. The jitter is kept out of
         * [faultBackoffMs] so the ladder's widening shape stays exactly
         * testable.
         */
        internal fun faultSleepMs(attempt: Int): Long {
            val base = faultBackoffMs(attempt)
            val spread = (base * 0.25).toLong()
            return base - spread + (Math.random() * (2 * spread + 1)).toLong()
        }
        ...
        /**
         * B6: the terminal message for a context window that stayed too large
         * after every shrink. Bilingual like the other terminal notices: it is
         * read by the user, not the model.
         */
        private const val CONTEXT_TOO_LARGE =
            "گفتگو برای پنجرهٔ زمینهٔ مدل جا نشد — حتی بعد از سه بار فشرده‌سازی. یک گفتگوی تازه شروع کنید یا کار را کوتاه‌تر کنید؛ تلاش دوباره به همین شکل ناموفق می‌ماند.\n" +
                "The conversation no longer fits the model's context window, even after compacting it 3 times. Start a new chat or shorten the task — retrying the same request would fail the same way."

        /**
         * Verification gate (Claude Code's verification loop): 3+ phases
         * closed with no verification since — the run is told to verify before
         * it declares victory.
         */
        private const val VERIFICATION_NUDGE =
            "[SYSTEM] 3 or more phases completed without verification since the last check. Before reporting completion, verify the work: spawn a verification sub-agent, or re-read the files you changed and confirm the behaviour end to end. Do not skip this."
    ...
    internal fun reasoningIntegrityRule(): String =
        ...
        /** One parseable candidate: its source span, its text, and an optional pre-parsed call. */
        private data class CallCandidate(
            val start: Int,
            val end: Int,
            val text: String,
            val preParsed: ToolCall?
        )

        /**
         * Every valid tool call in a message, in the order the model wrote
         * them — the multi-call path of the Claude Code tool loop.
         *
         * Kept separate from [parseToolCall] on purpose: the old method's
         * "last call wins" contract is load-bearing for MainActivity and for
         * the single-call fallback, so it stays byte-for-byte compatible while
         * this one collects everything.
         *
         * Candidates compete by SOURCE POSITION, not by kind. A fence body
         * that parses as one call (comments and trailing junk tolerated, like
         * the single-call path) claims its whole span: the balanced objects
         * nested inside it are the same call, not more calls. The old code
         * compared candidate STRINGS, so a fence body padded with newlines and
         * the object inside it were different strings, both parsed, and the
         * call ran TWICE. A fence whose body does not parse as one call
         * contributes nothing itself; the objects inside it are found by the
         * balanced scan like any other — which also keeps two objects in one
         * fence working instead of the first claiming the second's span.
         */
        fun parseToolCalls(raw: String?): List<ToolCall> {
            if (raw == null) {
                return emptyList()
            }
            val candidates = ArrayList<CallCandidate>()
            for (match in FENCE.findAll(raw)) {
                val body = match.groups[1] ?: continue
                val text = body.value
                // A fence body is ONE call only when it holds exactly one
                // top-level object: org.json parses the first object and
                // silently ignores trailing siblings, so a naive "it parsed"
                // check would claim the whole span for the first call and
                // swallow the second — the balanced scan below skips anything
                // nested inside a claimed span. When the body holds two calls,
                // no fence candidate is added and each is picked up
                // individually in source order.
                val direct = tryParseObject(text)
                val one: ToolCall?
                val parsedSource: String
                if (direct != null) {
                    one = direct
                    parsedSource = text
                } else {
                    val stripped = stripJsonComments(text) ?: text
                    one = tryParseObject(stripped)
                    parsedSource = stripped
                }
                if (one != null && extractBalancedSpans(parsedSource).size == 1) {
                    candidates.add(
                        CallCandidate(body.range.first, body.range.last + 1, text, one)
                    )
                }
            }
            for ((start, end) in extractBalancedSpans(raw)) {
                candidates.add(CallCandidate(start, end, raw.substring(start, end), null))
            }
            // Source order: the list follows the order the model wrote the
            // calls in. The sort is stable, so on a tied start (a fence body
            // beginning exactly with `{`) the fence — added first — wins and
            // claims the span.
            candidates.sortBy { it.start }
            val out = ArrayList<ToolCall>()
            val claimed = ArrayList<Pair<Int, Int>>()
            for (c in candidates) {
                // Overlapping source spans describe the same call: a fence
                // body claims its whole span, so the balanced object inside it
                // is skipped. Two IDENTICAL calls at different positions are
                // two calls — text dedup would silently drop the second.
                var nested = false
                for ((a, b) in claimed) {
                ...
                val call = c.preParsed ?: tryParse(c.text) ?: continue
                out.add(call)
                claimed.add(Pair(c.start, c.end))
            }
            return out
        }

        /**
         * The read-only tools whose calls are safe to run concurrently: no
         * writes, no deletions, no side effects beyond reading. A batch of
         * tool calls runs the ones in this set in parallel (max 4 workers)
         * and everything else serially in model order.
         */
        fun isConcurrencySafe(name: String?): Boolean = when (name) {
            Tools.ToolNames.READ_FILE,
            Tools.ToolNames.LIST_DIR,
            Tools.ToolNames.FILE_INFO,
            Tools.ToolNames.SEARCH,
            Tools.ToolNames.GLOB,
            Tools.ToolNames.WEB_SEARCH,
            Tools.ToolNames.WEB_FETCH,
            Tools.ToolNames.WEB_FETCH_MANY,
            Tools.ToolNames.READ_PDF,
            Tools.ToolNames.READ_ARCHIVE_ENTRY,
            Tools.ToolNames.LIST_ARCHIVE,
            Tools.ToolNames.RECALL -> true
            else -> false
        }

        /** Keyword detector for "think"/"ultrathink" style suffixes; word-boundary aware. */
        private val THINK_WORD = Regex("\\bthink\\b")

        /**
         * Name of the long-term memory file under the app's files dir. Mirrors
         * the private constant in Memory.kt — the prompt cache keys on its
         * mtime, so a `remember` mid-run rebuilds the prompt with the new fact.
         */
        private const val MEMORY_FILE_NAME = "agent_memory.txt"

        /**
         * Escalates the thinking level from magic keywords in the user's
         * message — Claude Code's keyword contract — or null when no keyword
         * is present. The ladder is strictly one-directional: the keyword can
         * only RAISE the configured level, never lower it (see
         * [AgentEngine.boostedThinkingLevel]).
         */
        internal fun thinkingKeywordLevel(text: String?): String? {
            val lower = (text ?: "").lowercase(Locale.US)
            if (lower.isBlankJava()) {
                return null
            }
            return when {
                lower.contains("ultrathink") ||
                    lower.contains("think harder") ||
                    lower.contains("think longer") -> "max"
                lower.contains("think hard") ||
                    lower.contains("megathink") -> "xhigh"
                THINK_WORD.containsMatchIn(lower) -> "high"
                else -> null
            }
        }

        /** Numeric rank of a thinking level for one-directional escalation. */
        private fun thinkingRank(level: String?): Int = when (level) {
            "low" -> 0
            "medium" -> 1
            "high" -> 2
            "xhigh" -> 3
        ...
        private fun extractBalancedObjects(text: String): ArrayList<String> {
            val out = ArrayList<String>()
            for ((start, end) in extractBalancedSpans(text)) {
        ...
        /**
         * Start (inclusive) / end (exclusive) spans of the top-level balanced
         * {...} objects in [text] — the same scan as [extractBalancedObjects],
         * keeping positions so callers can order and deduplicate by source
         * location instead of by string.
         */
        private fun extractBalancedSpans(text: String): ArrayList<Pair<Int, Int>> {
