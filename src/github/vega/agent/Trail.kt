...
class Workflow {
    ...
    fun launch(planIndex: Int, topic: String, fallbackTitle: String): WorkPhase =
        synchronized(this) {
            ...
            // B19: queueing time is not billable. The phase becomes RUNNING
            // here, but startedAt is stamped by the worker when it actually
            // picks the phase up — a row that was bound but never started
            // keeps startedAt 0 and honestly reads as "never started".
            phase.startedAt = 0L
    ...
    fun settle(interrupted: Boolean = false) {
        ...
        synchronized(this) {
            for (phase in phaseList) {
                ...
                // Backfill an end time for anything that was LAUNCHED and has none,
                ...
                //
                // B19 note: `launch()` deliberately leaves `startedAt` at 0 until a
                // worker actually picks the phase up, so the backfill must key on
                // `agentId` (real assigned work) rather than `startedAt`. A phase
                // that never started still bills zero — `durationMs` returns 0
                // when `startedAt <= 0L` — but a closed row must have an end time.
                if (phase.endedAt == 0L && (phase.startedAt > 0L || phase.agentId != 0)) {
                    phase.endedAt = now
                }
            }
        }
