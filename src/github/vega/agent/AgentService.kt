...
class AgentService : Service() {
    ...
    /**
     * Held for the whole run next to the CPU wake lock: a long download or
     * web-fetch batch must not have Wi-Fi throttled or dropped mid-transfer
     * when the screen is off. Released in [releaseWake], same lifecycle.
     */
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val worker = Thread({ runWorker(engine, chat, runId) }, "vega-agent-$runId")
        runThread = worker
        worker.start()
    ...
    private fun callback(chat: Chat, token: CancellationToken): AgentEngine.Callback =
        object : AgentEngine.Callback {
            ...
            override fun requestApproval(tool: String, args: JSONObject?): AgentBus.Decision =
                AgentBus.awaitApprovalDetailed(tool, args, token)
    ...
    private fun startForegroundSafe(): Boolean {
        ...
        return try {
            ...
            try {
                startForeground(NOTIF_ID, buildNotification(Fa.SVC_TEXT))
                true
    ...
    private fun buildNotification(text: String): Notification {
        ...
        builder.setContentTitle(Fa.SVC_TITLE)
            .setContentText(text)
            .setSmallIcon(R.mipmap.ic_stat_vega)
            .setContentIntent(open)
            .setOngoing(true)
            .setOnlyAlertOnce(true)
            .addAction(
                Notification.Action.Builder(
                    R.mipmap.ic_stat_vega, Fa.CANCEL, stopAction
                ).build()
            )
        try {
    ...
    private fun acquireWake() {
        synchronized(wakeLockLock) {
            try {
                ...
                val lock = power.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "vega:agent")
                wakeLock = lock
                ...
                // Wi-Fi at full performance for the run's lifetime: downloads
                // and long fetch batches stall when the radio dozes with the
                // screen off. Same try/catch discipline as the wake lock —
                // a missing WifiService must never break run startup.
                try {
                    val wifi = applicationContext.getSystemService(Context.WIFI_SERVICE)
                        as? WifiManager
                    val wlock = wifi?.createWifiLock(
                        WifiManager.WIFI_MODE_FULL_HIGH_PERF, "vega:agent"
                    )
    ...
    private fun releaseWake() {
        synchronized(wakeLockLock) {
            ...
            // The Wi-Fi lock shares the wake lock's lifecycle: whatever ends
            // the run releases both, and a null-then-release order keeps a
            // mid-release renew from resurrecting a dead lock.
            val wlock = wifiLock
            wifiLock = null
            try {
    ...
    companion object {
        const val CHANNEL_ID = "vega_agent_run"
        const val EXTRA_CHAT_ID = "chatId"
        const val EXTRA_RUN_ID = "runId"
        const val ACTION_STOP = "github.vega.agent.STOP_AGENT"
        ...
        /**
         * Starts the agent foreground service. Returns false (never throws)
         * when the platform refuses — on API 31+ a background
         * [Context.startForegroundService] throws
         * ForegroundServiceStartNotAllowedException, and one future caller
         * from a notification action or boot receiver must not crash the app.
         */
        fun start(context: Context, chatId: String, runId: Long): Boolean {
