...
class LlmClient private constructor(
    ...
    /**
     * Protocol hint, pinned only by tests. Production always passes null/auto:
     * the wire protocol is detected per endpoint (see [protocolCandidates]) —
     * there is no user-facing protocol setting any more.
     */
    private val provider: String = provider?.trimJava() ?: PROTOCOL_AUTO
    ...
    constructor(prefs: Prefs) : this(
        prefs.baseUrl(), prefs.apiKey(), prefs.model(), null, prefs.maxTokens(),
    ...
    /**
     * This request's protocol order: the instance hint (pinned by tests,
     * always auto in production) feeds the shared candidate ordering, so a
     * remembered or heuristically-guessed protocol leads and the other two
     * follow as fallbacks.
     */
    private fun orderedProtocols(): List<String> =
        protocolCandidates(provider, baseUrl, model)
    ...
    fun streamChat(messages: JSONArray, token: CancellationToken?, callback: StreamCallback) {
        ...
        router?.beginRequest()
        var attempt = 0
        // Automatic protocol detection: the endpoint's working protocol is
        // remembered per base URL, the static URL heuristic is the first guess
        // for a new endpoint, and the remaining protocols are fallbacks. There
        // is no protocol setting any more — if the first guess is wrong (e.g.
        // an Anthropic-shaped URL that actually speaks OpenAI), the failure's
        // mismatch signature moves the request to the next candidate instead
        // of surfacing an error.
        val protocols = orderedProtocols()
        var protocolIndex = 0
        ...
        while (!isCancelled(active, callback)) {
            ...
            try {
                streamAttempt(messages, active, callback, state, protocols[protocolIndex])
                rememberProtocol(baseUrl, protocols[protocolIndex])
                return
            } catch (cancelled: CancellationToken.CancelledException) {
                callback.onDone("")
                return
            } catch (error: LlmException) {
                // Protocol fallback, BEFORE the key router and the retry
                // budget: a 404/405 from the wrong wire format is not the
                // key's fault and not worth retrying on the same protocol.
                // Only while nothing streamed yet — partial content belongs
                // to the protocol that produced it and must not be discarded
                // for a re-send under another one.
                if (state.contentChars == 0 && isProtocolMismatch(error) &&
                    protocolIndex + 1 < protocols.size
                ) {
                    protocolIndex++
                    callback.onRetry()
                    attempt = 0
    ...
    private fun applyAuth(connection: HttpURLConnection, protocol: String) {
        ...
        if (lower.contains("openrouter.ai")) {
            connection.setRequestProperty("HTTP-Referer", "https://github.com/vega-agent/vega-android")
    ...
    internal fun completeOnce(messages: JSONArray, token: CancellationToken): String {
        // The Settings "test connection" button and the non-stream fallback
        // walk the protocol candidates: the first protocol whose endpoint
        // answers wins, so a misguessed protocol reports "works" instead of
        // a 404. Only mismatch-shaped failures move to the next candidate —
        // an auth or model error is terminal and surfaces as-is.
        var lastMismatch: LlmException? = null
        for (protocol in orderedProtocols()) {
            try {
                val result = completeOnceAs(messages, token, protocol)
                rememberProtocol(baseUrl, protocol)
                return result
            } catch (error: LlmException) {
        ...
        throw lastMismatch ?: LlmException(Fa.ERR_UNKNOWN, 0, false, 0L, "")
    }

    @Throws(Exception::class)
    private fun completeOnceAs(
        messages: JSONArray,
        token: CancellationToken,
        protocol: String
    ): String {
    ...
    companion object {
        ...
        /**
         * Hard ceiling on one serialized request body, in bytes. History is
         * compacted before assembly, but base64'd image attachments are not —
         * without this a multi-image turn could OOM the request buffer or
         * stall the upload with no error at all.
         */
        private const val MAX_REQUEST_BYTES = 24 * 1024 * 1024
        ...
        /**
         * The "no pin" protocol hint: resolve from the endpoint automatically
         * and fall back across protocols at runtime. This is what production
         * always uses now.
         */
        internal const val PROTOCOL_AUTO = "auto"
        ...
        internal fun endpointFor(
            ...
            var normalized = NetworkPolicy.normalizeUserEndpoint(
                trimTrailingSlashes(base?.trimJava() ?: "")
            )
            if (PROTOCOL_GEMINI == protocol) {
        ...
        internal fun resolveProtocol(selected: String?, base: String?, model: String?): String {
            if (PROTOCOL_ANTHROPIC == selected) {
                return PROTOCOL_ANTHROPIC
            }
            if (PROTOCOL_GEMINI == selected) {
                return PROTOCOL_GEMINI
            }
            if (PROTOCOL_OPENAI == selected) {
        ...
        /**
         * The protocols to try for an endpoint, in order: a positively detected
         * one first (see [rememberProtocol]), then the static URL-heuristic
         * guess, then the remaining protocols. The chat path walks this list
         * when the current protocol fails with a protocol-mismatch signature
         * (see [isProtocolMismatch]) — e.g. an Anthropic-shaped endpoint that
         * actually speaks OpenAI is found on the second attempt, with no user
         * involvement.
         */
        internal fun protocolCandidates(base: String?, model: String?): List<String> =
            protocolCandidates(PROTOCOL_AUTO, base, model)

        internal fun protocolCandidates(
            hint: String?,
            base: String?,
            model: String?
        ): List<String> {
            val ordered = ArrayList<String>(3)
            val cached = detectedProtocol(base)
            if (cached != null) {
                ordered.add(cached)
            }
            val first = resolveProtocol(hint, base, model)
            if (!ordered.contains(first)) {
                ordered.add(first)
            }
            for (p in arrayOf(PROTOCOL_OPENAI, PROTOCOL_ANTHROPIC, PROTOCOL_GEMINI)) {
        ...
        /**
         * True when the failure looks like "this endpoint does not speak the
         * protocol we used" rather than a problem with the request itself:
         * unknown path / wrong method on the wire format, or an explicit
         * unknown-endpoint error. Auth failures (401/403), rate limits (429)
         * and model problems are NOT mismatches — the endpoint understood the
         * protocol fine, so switching would only burn another request.
         */
        internal fun isProtocolMismatch(error: LlmException?): Boolean {
            if (error == null) {
                return false
            }
            val code = error.httpCode
            if (code == 404 || code == 405 || code == 501 || code == 410) {
                return true
            }
            if (code == 400) {
                val detail = error.serverDetail.lowercase(Locale.US)
                if (detail.contains("unknown request url") ||
                    detail.contains("unknown endpoint") ||
                    detail.contains("unsupported endpoint") ||
                    detail.contains("not a valid")
                ) {
                    return true
                }
            }
        ...
        /**
         * Remembers the protocol that demonstrably worked for a base URL, so
         * the next request starts there instead of re-discovering it.
         * In-memory and per-process: a stale entry only costs one failed
         * attempt before the fallback re-detects, which is exactly what would
         * have happened without the cache.
         */
        internal fun detectedProtocol(base: String?): String? =
            detectedByBase[normalizeBaseKey(base)]

        internal fun rememberProtocol(base: String?, protocol: String) {
            if (base.isNullOrBlankJava() || protocol.isEmpty()) {
                return
            }
            val key = normalizeBaseKey(base)
            val copy = HashMap(detectedByBase)
            copy[key] = protocol
            detectedByBase = copy
        }

        /** Test seam: clears the detected-protocol cache. */
        internal fun clearDetectedProtocols() {
            detectedByBase = emptyMap()
        }

        private fun normalizeBaseKey(base: String?): String {
            var key = base?.trimJava()?.lowercase(Locale.US) ?: ""
            while (key.endsWith("/")) {
        ...
        @Volatile
        private var detectedByBase: Map<String, String> = emptyMap()

        @Throws(Exception::class)
        private fun writeBody(connection: HttpURLConnection, body: JSONObject) {
            val bytes = body.toString().toByteArray(StandardCharsets.UTF_8)
            // History is compacted before assembly, but attached images are
            // base64'd whole — a 16 MB photo becomes ~21 MB of JSON — so a
            // multi-image turn could OOM the buffer or stall the upload with
            // no error at all. Fail loudly instead of sending a doomed body.
            if (bytes.size > MAX_REQUEST_BYTES) {
                throw LlmException(Fa.ERR_TOO_LARGE, 413, false, 0L, "")
            }
         ...
         *
         * The protocol is always auto-detected: [completeOnce] walks the
         * protocol candidates, so an endpoint whose URL guesses the wrong
         * protocol still reports success when another one answers.
         */
        fun testConnection(
            base: String?,
            key: String?,
            ...
            return try {
                ...
                // The provider argument is accepted for source compatibility and
                // ignored: detection is automatic now.
                val client = LlmClient(base, key, model, null, 512, 0.2f, "low", 0)
        ...
        fun testConnection(
            ...
            model: String?,
            anthropic: Boolean
        ): String? = testConnection(base, key, model, PROTOCOL_AUTO)
        ...
        private fun safeExceptionMessage(error: Exception): String {
            ...
            if (message.isNullOrBlankJava()) {
                return Fa.ERR_UNKNOWN
            }
            // Exception text can embed the request URL; a Gemini-style
            // `?key=SECRET` base URL would otherwise reach the error card
            // verbatim. Host and path stay, query and fragment go.
            return Util.redactUrlsInText(message)
        }
