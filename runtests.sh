...
#                point VEGA_JAVA_SRC at it (or pass it after --diff).
#
#   4. jvm       tests/github/vega/agent/CoreRegressionTests.kt
...
if [[ "$MODE" != "jvm" ]]; then
  if ! "$ROOT/tools/find-kotlinc.sh" >/dev/null 2>&1; then
    ...
    if [[ -d "${JAVA_SRC:-}" ]]; then
      ...
      skip diff "original Java tree not found (pass its src dir, or set VEGA_JAVA_SRC)"
    fi
    if [[ $run_diff == 1 ]]; then
      banner "offline compile + differential test vs the original Java"
      "$ROOT/tools/build-offline.sh" --java "$JAVA_SRC" --fuzz "${VEGA_FUZZ:-600}"
      pass offline
      pass diff
    else
...
if resolve_jvm_prereqs; then
  ...
  BUILD_DIR="${VEGA_TEST_DIR:-$ROOT/.build-tests}"
  rm -rf "$BUILD_DIR"
  ...
  echo "$BUILD_DIR/gen/github/vega/agent/R.java" >> "$BUILD_DIR/sources.txt"
  ...
  "$JAVA_HOME/bin/java" \
    ...
    github.vega.agent.CoreRegressionTests
  pass jvm
