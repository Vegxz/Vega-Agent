...
#   VEGA_ANDROID_JAR    path to android.jar (default: sdk/platforms or ./sdk/)
#   VEGA_R8_JAR         path to r8.jar      (default: $BT/d8 or ./sdk/r8.jar)
#   VEGA_KEYSTORE_PATH  release .jks        (required)
#   VEGA_KEYSTORE_PASSWORD / VEGA_KEY_PASSWORD / VEGA_KEY_ALIAS=vega
#
...
KOTLIN_STDLIB="${VEGA_KOTLIN_STDLIB:-$KOTLIN_HOME/lib/kotlin-stdlib.jar}"
[[ -e "$KOTLIN_STDLIB" ]] || { echo "kotlin-stdlib.jar not found at $KOTLIN_STDLIB" >&2; exit 2; }

BT=""
declare -a bt_candidates=()
if [[ -n "${ANDROID_BUILD_TOOLS_VERSION:-}" ]]; then
  # Explicit pin wins.
  bt_candidates+=( "${ANDROID_SDK_ROOT:-}/build-tools/$ANDROID_BUILD_TOOLS_VERSION" )
elif [[ -d "${ANDROID_SDK_ROOT:-/nonexistent}/build-tools" ]]; then
  # No pin: take the newest installed build-tools. The old default (35.0.1)
  # broke on any machine that had only 35.0.0 installed.
  while IFS= read -r dir; do
    bt_candidates+=( "$dir" )
  done < <(ls -d "${ANDROID_SDK_ROOT}/build-tools/"*/ 2>/dev/null | sort -Vr)
fi
bt_candidates+=(
  /usr/lib/android-sdk/build-tools/debian
  "$HOME/android-sdk/build-tools/35.0.1"
)
for candidate in "${bt_candidates[@]}"; do
...
BUILD_DIR="${VEGA_BUILD_DIR:-$ROOT/.build-release}"
VNAME="${1:-1.1.0}"
# Default version code tracks build.gradle.kts (the single source of truth), so
# a local ./mkapk.sh build keeps rising past the last release instead of
# shipping code 1, which Android would refuse to install over it.
GRADLE_VCODE="$(sed -n 's/^[[:space:]]*versionCode = \([0-9][0-9]*\).*/\1/p' "$ROOT/build.gradle.kts" 2>/dev/null | head -1)"
VCODE="${2:-${GRADLE_VCODE:-17}}"
...
: "${VEGA_KEYSTORE_PATH:?Set VEGA_KEYSTORE_PATH to the release .jks file}"
: "${VEGA_KEYSTORE_PASSWORD:?Set VEGA_KEYSTORE_PASSWORD securely in the environment}"
VEGA_KEY_PASSWORD="${VEGA_KEY_PASSWORD:-$VEGA_KEYSTORE_PASSWORD}"
VEGA_KEY_ALIAS="${VEGA_KEY_ALIAS:-vega}"

D8=()
if [[ -n "$BT" && -x "$BT/d8" ]]; then
  D8=("$BT/d8")
else
  R8_JAR="${VEGA_R8_JAR:-$ROOT/sdk/r8.jar}"
  if [[ -e "$R8_JAR" ]]; then
    D8=("$JAVA_HOME/bin/java" "-cp" "$R8_JAR" "com.android.tools.r8.D8")
  fi
fi

for required in "$JAVA_HOME/bin/javac" "$ANDROID_JAR" "$VEGA_KEYSTORE_PATH"; do
  if [[ ! -e "$required" ]]; then
...
[[ ${#D8[@]} -gt 0 ]] || { echo "d8 not found; set VEGA_R8_JAR or install build-tools" >&2; exit 2; }
...
printf '== sign release ==\n'
SIGN_ARGS=(
  --ks "$VEGA_KEYSTORE_PATH"
  --ks-key-alias "$VEGA_KEY_ALIAS"
  --ks-pass "env:VEGA_KEYSTORE_PASSWORD"
...
if [[ "$VEGA_KEY_PASSWORD" != "$VEGA_KEYSTORE_PASSWORD" ]]; then
  SIGN_ARGS=(
    --ks "$VEGA_KEYSTORE_PATH"
    --ks-key-alias "$VEGA_KEY_ALIAS"
    --ks-pass "env:VEGA_KEYSTORE_PASSWORD"
    --key-pass "env:VEGA_KEY_PASSWORD"
    --v1-signing-enabled true --v2-signing-enabled true --v3-signing-enabled true
    --min-sdk-version 23
    --out "$OUT" "$BUILD_DIR/out/aligned.apk"
  )
