...
public class JSONObject {
     ...
     * ships on. The offline harness previously lacked this overload, so any
     * app code calling it failed to compile here while working fine on device.
     */
    ...
    StringBuilder writeIndented(StringBuilder sb, int indentFactor, int indent) {
        ...
        for (Map.Entry<String, Object> e : map.entrySet()) {
            if (!first) { sb.append(",\n"); }
            first = false;
            pad(sb, indent + indentFactor);
            sb.append(quote(e.getKey())).append(": ");
            writeValueIndented(sb, e.getValue(), indentFactor, indent + indentFactor);
        }
        sb.append('\n');
        pad(sb, indent);
        return sb.append('}');
    ...
    static void writeValueIndented(StringBuilder sb, Object value, int indentFactor, int indent) {
        if (value instanceof JSONObject) {
            ((JSONObject) value).writeIndented(sb, indentFactor, indent);
        } else if (value instanceof JSONArray) {
            ((JSONArray) value).writeIndented(sb, indentFactor, indent);
        } else {
            sb.append(encode(value));
        }
    }

    static void pad(StringBuilder sb, int count) {
