...
OUT="${1:-vega-release-private.jks}"
ALIAS="${2:-vega}"
PASS="${3:-$(head -c 24 /dev/urandom | base64 | tr -dc 'A-Za-z0-9' | cut -c1-32)}"
DNAME="${VEGA_DNAME:-CN=Vega, OU=Mobile, O=Vega, L=Tehran, C=IR}"
DAYS="${VEGA_VALIDITY_DAYS:-10950}"      # 30 years
...
Next:
  export VEGA_KEYSTORE_PATH="\$PWD/$OUT"
  export VEGA_KEYSTORE_PASSWORD='$PASS'
  export VEGA_KEY_ALIAS='$ALIAS'
