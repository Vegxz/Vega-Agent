...
OUT="${VEGA_OFFLINE_DIR:-$ROOT/.build-offline}"
JAVA_SRC=""
FUZZ=600
RUN_TESTS=0
...
R_JAVA="$OUT/gen/github/vega/agent/R.java"
...
if [[ $RUN_TESTS == 1 ]]; then
  ...
  env JAVA_TOOL_OPTIONS= java -Dstdout.encoding=UTF-8 \
      -cp "$OUT/test-classes:$SUPPORT:$STDLIB" github.vega.agent.CoreRegressionTests
  ...
  env JAVA_TOOL_OPTIONS= java -Dstdout.encoding=UTF-8 \
      -cp "$OUT/test-classes:$SUPPORT:$STDLIB" github.vega.agent.AgentLoopTests
fi
