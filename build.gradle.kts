...
val keystoreProperties: Map<String, String> = run {
    ...
    // Map the file's own key names onto the VEGA_* names used everywhere else.
    mapOf(
        "VEGA_KEYSTORE_PATH" to loaded.getProperty("storeFile"),
        "VEGA_KEYSTORE_TYPE" to loaded.getProperty("storeType"),
        "VEGA_KEYSTORE_PASSWORD" to loaded.getProperty("storePassword"),
        "VEGA_KEY_ALIAS" to loaded.getProperty("keyAlias"),
        "VEGA_KEY_PASSWORD" to loaded.getProperty("keyPassword"),
    ).filterValues { it != null }.mapValues { it.value as String }
}

android {
    namespace = "github.vega.agent"
    compileSdk = 35

    defaultConfig {
        applicationId = "github.vega.agent"
        ...
        minSdk = 23
        targetSdk = 35
        // Version 1.1.0. The displayed version is "1.1.0" everywhere it appears — this
        ...
        // VEGA_KEYSTORE_PATH. Android refuses an update signed by a different key
    ...
    signingConfigs {
        create("release") {
            ...
            val path = setting("VEGA_KEYSTORE_PATH")
            val store = path?.let(::file)
            if (store != null && store.isFile) {
                storeFile = store
                storeType = setting("VEGA_KEYSTORE_TYPE", "PKCS12")
                storePassword = setting("VEGA_KEYSTORE_PASSWORD")
                keyAlias = setting("VEGA_KEY_ALIAS", "vega")
                keyPassword = setting("VEGA_KEY_PASSWORD")
                    ?: setting("VEGA_KEYSTORE_PASSWORD")
            } else {
                logger.lifecycle(
