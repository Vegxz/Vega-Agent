...
SRC = ROOT / "src/github/vega/agent"
...
def test_multi_delete_contract():
    ...
    tools = read("Tools.kt")
    require("\"paths\"" in tools and "deleteSinglePath" in tools and "pathListArg" in tools,
            "Tools does not safely execute a paths array")
    require('delete_path requires' in tools, "empty delete path guard missing")
...
def test_direction_and_instant_settings():
    ...
    for setter in ("setThinkingLevel", "setThemeMode", "setWebSearch"):
        require(f"prefs.{setter}" in settings, f"instant setting missing: {setter}")
    # The protocol picker is gone on purpose: the wire protocol is detected
    # automatically per endpoint, so no setter, no stored provider, and no
    # protocol UI may remain.
    require("setProvider" not in settings,
            "protocol setter still exists in SettingsActivity")
    require("prefs.provider()" not in settings,
            "SettingsActivity still reads a stored provider")
    require("PROV_AUTO" not in strip_comments(prefs_src) and
            "PROV_OPENAI" not in strip_comments(prefs_src),
            "Prefs still defines protocol constants")
...
def test_protocol_auto_detection_and_fallback():
    client = read("LlmClient.kt")
    bare = strip_comments(client)
    # The candidates, the mismatch detector and the remember-on-success cache
    # must all exist and be WIRRED: streamChat walks the candidates and
    # completeOnce (the connection test) does too.
    for name in ("protocolCandidates", "isProtocolMismatch", "rememberProtocol"):
        require(name in bare, f"LlmClient helper missing: {name}")
    require("orderedProtocols()" in bare,
            "streamChat does not walk protocol candidates")
    require("for (protocol in orderedProtocols())" in bare,
            "completeOnce does not walk protocol candidates")
    # The mismatch detector only fires on unknown-path/wrong-method shapes
    # (404/405/501/410, some 400s); 401/429 fall through to false because the
    # endpoint understood the protocol — the failure is key/rate, not format.
    require("code == 404 || code == 405 || code == 501 || code == 410" in bare,
            "protocol mismatch detector lost its unknown-path shape")
    require("isProtocolMismatch(error)" in bare,
            "streamChat fallback does not consult the mismatch detector")
    settings = read("SettingsActivity.kt")
    # No protocol UI anywhere: no segment, no pills, no stored choice.
    for token in ("protocolSegment", "protoPills", "selProtocol"):
        require(token not in strip_comments(settings),
                f"protocol UI remnant in SettingsActivity: {token}")
    require("Fa.SET_PROTOCOL" not in settings,
            "SettingsActivity still references the protocol section label")
    fa = read("Fa.kt")
    require("SET_PROTOCOL:" not in fa and "SET_PROTO_AUTO" not in fa,
            "Fa still defines protocol-picker strings")


def test_models_fetched_only_after_key():
    settings = read("SettingsActivity.kt")
    bare = strip_comments(settings)
    # The fetch refuses to touch the network before a key exists…
    require("Fa.SET_MODELS_NEED_KEY" in bare,
            "model fetch has no enter-key-first gate")
    require("if (effKey.isEmpty())" in bare,
            "model fetch does not gate on an empty key")
    # …and a newly entered key triggers the fetch by itself, opening the
    # picker when models come back.
    require("keyAutoFetchDebounced" in bare,
            "no auto-fetch after the key is entered")
    require("fetchModels(autoExpand = true)" in bare,
            "key auto-fetch does not open the picker on success")
    # The candidate loop (not a single guessed protocol) serves the list.
    require("for (protocol in candidates)" in bare,
            "model fetch does not walk protocol candidates")
...
def test_key_router_and_reset_contracts():
    ...
    require('getInt("max_tokens"' in prefs, "max tokens pref missing")
    require("defaultMaxTokens()" in prefs and "ModelCaps.forModel(model())" in prefs,
            "max tokens default should follow the selected model's ceiling")
    settings = read("SettingsActivity.kt")
...
def test_all_java_files_were_ported():
    java_dir = ROOT.parent / "VegaCode-v1-src/src/github/vega/agent"
...
def read_test(name):
    return (ROOT / "tests/github/vega/agent" / name).read_text(encoding="utf-8")
...
def test_single_tap_shows_no_copy_panel():
    """
    A single tap on a message must do nothing: the tap-to-copy inline panel
    was removed at the user's request. Long-press still opens the
    Select / Copy / Share panel via MessagePanel.
    """
    main = read("MainActivity.kt")
    code = strip_comments(main)
    require("private fun attachCopyPanel(" not in code,
            "tap-to-copy installer still present")
    require("toggleCopyPanel" not in code,
            "tap-to-copy toggle still present")
    require("openCopyPanel" not in code,
            "tap-to-copy panel field still present")
    require("hideCopyPanel" not in code,
            "tap-to-copy hide helper still present")
    require("buildCopyPanel" not in code,
            "tap-to-copy panel builder still present")
    # The long-press panel must still be attached to message leaves.
    require("MessagePanel.attach" in code,
            "long-press Select/Copy/Share panel is no longer attached")
    panel = read("MessagePanel.kt")
    require("setOnLongClickListener" in panel,
            "MessagePanel no longer installs its long-press listener")


def test_select_covers_the_whole_message():
    """
    "Select" must collect the whole message, not the one leaf under the
    finger: the sheet gathers every prose leaf under the message root, which
    the walker threads through from the message container.
    """
    panel = read("MessagePanel.kt")
    require("collectMessageText" in panel,
            "whole-message collection missing from MessagePanel")
    require("messageRoot" in panel,
            "MessagePanel no longer threads the message root through")
    main = read("MainActivity.kt")
    require("attachMessagePanels" in main,
            "message-panel walker missing from MainActivity")
    require("MessagePanel.attach(this, child, textProvider, root)" in main,
            "the walker does not hand the message root to the panel")


def test_formula_typography_is_applied():
    """
    Math and chemistry render in the bundled serif math face (Latin Modern
    Math, the classic TeX look):
    the font asset ships, the typography pass exists, and every rendered
    message path goes through it.
    """
    assets = Path("assets/fonts/LatinModernMath-Regular.otf")
    require(assets.exists(), "Latin Modern Math font asset missing")
    require(assets.stat().st_size > 50000, "Latin Modern Math font asset is suspiciously small")
    code = read("FormulaTypography.kt")
    require("fun beautify" in code, "FormulaTypography.beautify missing")
    require("StackedFractionSpan" in code, "stacked fractions missing")
    require("SubscriptSpan" in code and "SuperscriptSpan" in code,
            "true sub/superscript spans missing")
    renderer = read("MarkdownRenderer.kt")
    require("FormulaTypography.beautify" in renderer,
            "rendered messages do not go through the typography pass")
    app = read("App.kt")
    require("FormulaTypography.init" in app, "typeface is never initialised")


def test_plan_ui_is_bound_to_its_chat():
    """
    A plan sheet, its actions and its wanted-changes signal belong to the
    chat whose run produced them — never to whatever chat happens to be open.
    """
    main = read("MainActivity.kt")
    require("runChatId" in main, "finished-run chat is not tracked")
    require("if (chat?.id == runChatId)" in main,
            "maybeShowPlan is not gated on the finished run's chat")
    require("sheetChatId" in main, "plan sheet does not capture its chat id")
    require("answerPlan(sheetChatId" in main,
            "plan answers are not bound to the sheet's chat")


def test_downloads_stage_durably_and_resume():
    """
    Large downloads stage outside cache (which the system may clear), keep
    the partial file on cancellation with resume metadata, validate resumed
    ranges, and scale the deadline to the file size.
    """
    tools = read("Tools.kt")
    require("dl_staging" in tools, "durable download staging missing")
    require("keepStaged" in tools, "cancelled downloads do not keep their partial file")
    require("rangeStartOk" in tools, "resumed Content-Range is not validated")
    require("total / 50" in tools, "download deadline is not scaled to file size")
    require("usableSpace" in tools, "no disk-space precheck before downloading")
...
def test_window_chrome_matches_the_palette():
    ...
    for styles in ("res/values/styles.xml", "res/values-v29/styles.xml"):
        ...
        # The two grounds: dark #FF0D0D0D and light #FFF2F2F7 (iOS grouped grey).
        for literal in ("#FF0D0D0D", "#FFF2F2F7"):
    ...
    require("BG = 0xFF0D0D0D" in theme and "BG = 0xFFF2F2F7" in theme,
...
def test_timeout_is_a_real_deadline():
    ...
    require("vega-http-deadline" in client, "the non-streaming path has no hard deadline")
...
def test_dynamic_workflow():
    ...
    require("MAX_SUBAGENT_DEPTH" in engine and "maxSubagentStepsForLevel" in engine,
...
def test_palette_is_strictly_monochrome():
    ...
    Two sanctioned exceptions, both excluded by NAME rather than by value so
    neither can quietly widen:
      - the diff colours (`DIFF_ADD` / `DIFF_DEL` and their grounds): added and
        removed are opposites, not two amounts of one thing, and lightness alone
        cannot say which is which;
      - the light page ground (`BG` / `BG_ELEV`): iOS's systemGray6, whose blue
        channel runs five steps ahead of red and green;
      - the settings menu's icon tiles (`tileBlue` et al): iOS's own system
        hues in the register of iOS Settings.

    Two rules:
      1. Every other 8-digit ARGB literal in Theme.kt must satisfy R == G == B.
    ...
    # The DIFF_* lines are the first sanctioned exception, and they are excluded
    # by NAME rather than by value so the exception cannot quietly widen: added
    # and removed are opposites, not two amounts of one thing, and lightness
    # alone cannot say which is which. Every other literal in the file must be
    # grey — except the two further exceptions handled in the loop below.
    hued_ok = 0
    for line in theme.splitlines():
        if re.search(r"\bDIFF_(ADD|DEL)(_BG)?\s*=", line):
            hued_ok += 1
            continue
        # The light page ground is iOS's systemGray6 (#FFF2F2F7): red and green
        # match while blue runs five steps ahead, so the strict grey check below
        # would flag it. Excluded by NAME — like the diff colours — so the
        # exception cannot quietly widen to the rest of the palette.
        if re.search(r"^\s*BG(_ELEV)?\s*=", line):
            continue
        # The settings menu's icon tiles wear iOS's own system hues, in the
        # register of iOS Settings (a solid coloured tile with a white glyph).
        # Excluded by NAME for the same reason: a new tile colour is a deliberate
        # addition to this family, never a stray literal somewhere else.
        if re.search(r"\bfun tile[A-Z]\w*\(\)", line):
            continue
...
def test_app_is_branded_vega():
    ...
    So this bans the display forms (`Vega Code`, `VegaCode`) and deliberately
    tolerates the snake/dotted ones (vega_prefs, vega_apikey_v1,
    vega_agent_run, vega_chat_id, vega_code_card, vega-llm-io,
    github.vega.agent, ic_stat_vega).
    """
    ...
    #
    # There used to be two keys: HEADER_BRAND said "Vega Agent" and APP_NAME said
    ...
    stale = re.compile(r"Vega[ ]Code|VegaCode")
    offenders = []
...
def test_a_folded_step_is_not_drawn_twice():
    ...
    engine = read("AgentEngine.kt")
    body = _function_body(engine, "    fun run(chat: Chat, token: CancellationToken, callback: Callback) {")
    parse_at = body.index("val calls = parseToolCalls(message.content)")
...
def _sources():
    import os
    return set(os.listdir(os.path.join(os.path.dirname(__file__), "..", "src", "github", "vega", "agent")))
...
def test_the_agent_loop_is_actually_tested():
    ...
    require("github.vega.agent.AgentLoopTests" in offline,
...
def test_the_displayed_version_is_1_1_0():
    """
    Every surface that names a version says 1.1.0.
    ...
    require('versionName = "1.1.0"' in gradle, "the build does not report version 1.1.0")
    workflow = read_root(".github/workflows/android.yml")
    require('./mkapk.sh "1.1.0"' in workflow, "CI still builds a different version")
    require("Vega-v1.1.0.apk" in workflow, "CI does not name its artifact v1.1.0")
    require("Vega-v5" not in workflow, "CI still names its artifact v5")
    require("Vega-v1.apk" not in workflow, "CI still names its artifact v1")
    fa = read("Fa.kt")
    require('val SET_VERSION: String get() = "v1.1.0"' in fa,
    ...
    require(code is not None and int(code.group(1)) >= 17,
...
def test_no_signing_material_is_in_the_tree():
    ...
    fields = ("storePassword", "keyPassword", "VEGA_KEYSTORE_PASSWORD")
...
def test_router_key_rows_have_copy_button():
    """Every stored router-key row offers a copy button for the full key."""
    settings = read("SettingsActivity.kt")
    require('Ui.iconButton(this, "copy", 18.0f, Theme.TEXT_MUTED)' in settings,
            "router key row copy button missing")
    require("MarkdownRenderer.copyText(this, Fa.SET_API_KEY, key)" in settings,
            "router key copy does not copy the full key")


def test_chat_menu_has_json_copy_row():
    """The history-panel menu offers a JSON export of the conversation."""
    main = read("MainActivity.kt")
    require("Fa.CHAT_MENU_COPY_JSON" in main, "chat menu JSON row missing")
    require("copyChatJson(targetId)" in main, "chat menu JSON action missing")
    require("target.toJson().toString(2)" in main, "JSON export is not indented")
    fa = read("Fa.kt")
    require("CHAT_MENU_COPY_JSON: String get()" in fa,
            "CHAT_MENU_COPY_JSON string missing")


def test_skills_list_is_collapsible():
    """Installed skills live in an expandable/collapsible list."""
    settings = read("SettingsActivity.kt")
    require("Fa.SKILL_LIST" in settings, "skills list header missing")
    require("toggleSkillsExpanded()" in settings,
            "skills expand/collapse toggle missing")
    require("skillsExpanded" in settings, "skills expanded state missing")
    fa = read("Fa.kt")
    require("SKILL_LIST: String get()" in fa, "SKILL_LIST string missing")


def test_system_prompt_names_vega_agent_in_persian():
    """The system prompt teaches the model both names: وگا ایجنت and Vega Agent."""
    engine = read("AgentEngine.kt")
    require("وگا ایجنت" in engine,
            "Persian agent name missing from the system prompt")
    require("Vega Agent" in engine,
            "English agent name missing from the system prompt")


def test_max_tokens_label():
    """The response-token field is labelled حداکثر توکن."""
    fa = read("Fa.kt")
    require("حداکثر توکن" in fa, "حداکثر توکن label missing")
    require("بیشینه توکن پاسخ" not in fa, "old response-token label still present")


def test_long_press_panel_replaces_direct_selection():
    """
    Long-pressing a message opens the Select / Copy / Share panel instead of
    starting text selection directly. The panel floats as a PopupWindow (kept
    out of MainActivity, where the tap-to-copy contract still bans popups),
    Copy reuses the hardened clipboard path, Share fires ACTION_SEND with the
    text in EXTRA_TEXT, and Select opens the whole message — prose leaves and
    code bodies alike — in a selectable sheet, so the selection handles can
    span the message instead of one rendered block. A bare performLongClick()
    could only ever select the single block under the finger, which is exactly
    what "Select covers the entire message" forbids.
    """
    panel = read("MessagePanel.kt")
    for action in ('"select"', '"copy"', '"share"'):
        require(action in panel, f"the long-press panel lost its {action} action")
    require("Fa.SELECT" in panel and "Fa.SHARE" in panel,
            "panel actions are not localized through Fa")
    require("Intent.ACTION_SEND" in panel, "share does not use ACTION_SEND")
    require("EXTRA_TEXT" in panel, "share does not put the message in EXTRA_TEXT")
    require("setTextIsSelectable(true)" in panel,
            "Select no longer opens a selectable sheet")
    require("collectMessageText" in panel,
            "Select lost its whole-message collector")
    require("TAG_CODE_BODY" in panel,
            "Select skips code blocks instead of covering the entire message")
    require("MarkdownRenderer.copyText" in panel,
            "panel copy bypasses the hardened clipboard path")
    require("isOutsideTouchable" in panel, "the panel cannot be dismissed by an outside tap")
    main = read("MainActivity.kt")
    require("MessagePanel.attach" in main, "the panel is never attached to messages")
    # The tap-to-copy contract still holds: MainActivity itself must not pop up windows.
    require("PopupWindow(" not in strip_comments(main), "MainActivity grew its own popup")


def test_install_skill_tool_is_wired_end_to_end():
    """
    install_skill is a real tool, not a documented fantasy: the name is
    registered, it is classified as mutating (so ACCEPT mode gates it), the
    dispatcher routes it, and the system prompt teaches the model the
    search -> fetch -> install loop with real SKILL.md files.
    """
    tools = read("Tools.kt")
    require('INSTALL_SKILL = "install_skill"' in tools, "the tool name is not declared")
    require("ToolNames.INSTALL_SKILL" in strip_comments(tools).replace(
        "INSTALL_SKILL = ", ""),
        "install_skill is declared but never referenced")
    engine = read("AgentEngine.kt")
    require("install_skill" in engine, "the model is never told install_skill exists")
    require("SKILL.md" in engine, "the prompt does not teach the SKILL.md workflow")


def test_math_notation_renders_without_a_tex_engine():
    """
    Math is translated to Unicode, never rendered by a WebView: inline $..$
    and \\(..\\) lift before emphasis (so * and _ inside math stay literal),
    $$..$$ / \\[..\\] become centred blocks, and "$5 and $10" is not math.
    """
    renderer = read("MarkdownRenderer.kt")
    require("fun latexToUnicode" in renderer, "the LaTeX-to-Unicode translator is gone")
    require("addMathBlock" in renderer, "display math has no centred block")
    require("liftMathSpans" in renderer, "inline math is not lifted before emphasis")
    require("$5 and $10" in renderer, "the currency guard lost its documented example")
    require("VULGAR_FRACTIONS" in renderer, "common fractions have no vulgar forms")
    require("SUPERSCRIPT" in renderer and "SUBSCRIPT" in renderer,
            "super/subscripts are not translated")
...
def main():
    ...
    print(f"source regressions: {passed} passed, {len(failures)} failed, {len(tests)} total")
    if failures:
        raise SystemExit(1)
